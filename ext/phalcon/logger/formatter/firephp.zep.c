...
PHP_METHOD(Phalcon_Logger_Formatter_Firephp, format) {
	...
	zephir_read_property(&_10, this_ptr, SL("_enableLabels"), PH_NOISY_CC | PH_READONLY);
	_11 = !zephir_is_true(&_10);
	if (_11) {
		zephir_read_property(&_12, this_ptr, SL("_showBacktrace"), PH_NOISY_CC | PH_READONLY);
		_11 = !zephir_is_true(&_12);
	}
	zephir_read_property(&_13, this_ptr, SL("_enableLabels"), PH_NOISY_CC | PH_READONLY);
	_14 = zephir_is_true(&_13);
	if (_14) {
		zephir_read_property(&_15, this_ptr, SL("_showBacktrace"), PH_NOISY_CC | PH_READONLY);
	...
	if (_11) {
		...
		ZEPHIR_INIT_NVAR(&body);
		array_init(&body);
		zephir_read_property(&_16$$12, this_ptr, SL("_showBacktrace"), PH_NOISY_CC | PH_READONLY);
		if (zephir_is_true(&_16$$12)) {
			zephir_array_update_string(&body, SL("backtrace"), &backtrace, PH_COPY | PH_SEPARATE);
		}
		zephir_read_property(&_17$$12, this_ptr, SL("_enableLabels"), PH_NOISY_CC | PH_READONLY);
