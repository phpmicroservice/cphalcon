...
PHP_METHOD(Phalcon_Logger_Formatter_Line, __construct) {
	...
	zephir_fetch_params_without_memory_grow(0, 2, &format, &dateFormat);
...
PHP_METHOD(Phalcon_Logger_Formatter_Line, format) {
	...
	if (zephir_memnstr_str(&format, SL("%date%"), "phalcon/logger/formatter/line.zep", 80)) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$3, "date", NULL, 287, &_1$$3, &_2$$3);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_4$$3);
		ZVAL_STRING(&_4$$3, "%date%");
		zephir_fast_str_replace(&_0$$3, &_4$$3, &_3$$3, &format);
	...
	if (zephir_memnstr_str(&format, SL("%type%"), "phalcon/logger/formatter/line.zep", 87)) {
		...
		zephir_fast_str_replace(&_5$$4, &_8$$4, &_6$$4, &format);
		ZEPHIR_CPY_WRT(&format, &_5$$4);
	}
	ZEPHIR_INIT_VAR(&_9);
	ZEPHIR_INIT_VAR(&_10);
	ZVAL_STRING(&_10, "%message%");
	zephir_fast_str_replace(&_9, &_10, &message, &format);
	ZEPHIR_INIT_VAR(&_11);
