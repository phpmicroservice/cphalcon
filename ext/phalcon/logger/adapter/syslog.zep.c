...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, __construct) {
	...
	if (zephir_is_true(name)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "openlog", NULL, 283, name, &option, &facility);
...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, logInternal) {
	...
	if (Z_TYPE_P(&appliedFormat) != IS_ARRAY) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_logger_exception_ce, "The formatted message is not valid", "phalcon/logger/adapter/syslog.zep", 101);
		return;
	}
	zephir_array_fetch_long(&_3, &appliedFormat, 0, PH_NOISY | PH_READONLY, "phalcon/logger/adapter/syslog.zep", 104);
	zephir_array_fetch_long(&_4, &appliedFormat, 1, PH_NOISY | PH_READONLY, "phalcon/logger/adapter/syslog.zep", 104);
	ZEPHIR_CALL_FUNCTION(NULL, "syslog", NULL, 284, &_3, &_4);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
