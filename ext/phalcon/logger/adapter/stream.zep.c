...
PHP_METHOD(Phalcon_Logger_Adapter_Stream, __construct) {
	...
	zval *name_param = NULL, *options = NULL, options_sub, __$null, mode, stream, _0$$6;
	zval name, _1$$6;
	...
	if (!(zephir_is_true(&stream))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$6, "phalcon/logger/adapter/stream.zep", 76);
		ZEPHIR_MM_RESTORE();
		return;
	}
