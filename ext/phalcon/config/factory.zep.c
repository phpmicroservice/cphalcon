...
PHP_METHOD(Phalcon_Config_Factory, loadClass) {
	...
	if (Z_TYPE_P(config) == IS_STRING) {
		...
		ZVAL_STRING(&_0$$3, ".");
		ZEPHIR_CALL_FUNCTION(&_1$$3, "strrchr", NULL, 137, config, &_0$$3);
		zephir_check_call_status();
		ZVAL_LONG(&_2$$3, 1);
	...
	if (_4) {
		_4 = zephir_instance_of_ev(config, phalcon_config_ce);
	}
	if (_4) {
	...
	if (zephir_array_isset_string_fetch(&adapter, config, SL("adapter"), 0)) {
		...
		if (!(zephir_is_true(&_8$$8))) {
			ZEPHIR_CALL_FUNCTION(&_9$$9, "lcfirst", NULL, 64, &adapter);
			zephir_check_call_status();
