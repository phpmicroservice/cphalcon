...
PHP_METHOD(Phalcon_Mvc_Model_Binder, bindToHandler) {
	...
	zval cacheKey;
	zval params;
	zval *handler, handler_sub, *params_param = NULL, *cacheKey_param = NULL, *methodName = NULL, methodName_sub, __$null, paramKey, className, boundModel, paramsCache, paramValue, _0, _2$$3, *_3$$4, _4$$4;
	zval *this_ptr = getThis();
	...
	if (_1) {
		...
		array_init(&_2$$3);
		zephir_update_property_zval(this_ptr, SL("boundModels"), &_2$$3);
		ZEPHIR_CALL_METHOD(&paramsCache, this_ptr, "getparamsfromcache", NULL, 0, &cacheKey);
		zephir_check_call_status();
		if (Z_TYPE_P(&paramsCache) == IS_ARRAY) {
			zephir_is_iterable(&paramsCache, 0, "phalcon/mvc/model/binder.zep", 103);
			if (Z_TYPE_P(&paramsCache) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&paramsCache), _5$$4, _6$$4, _3$$4)
				{
					...
					zephir_array_fetch(&paramValue, &params, &paramKey, PH_NOISY, "phalcon/mvc/model/binder.zep", 96);
...
PHP_METHOD(Phalcon_Mvc_Model_Binder, getParamsFromReflection) {
	...
	zend_class_entry *_9$$10, *_17$$25;
	zend_string *_4;
	zend_ulong _3;
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zephir_fcall_cache_entry *_8 = NULL, *_10 = NULL, *_13 = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval cacheKey;
	zval params;
	zval *handler, handler_sub, *params_param = NULL, *cacheKey_param = NULL, *methodName, methodName_sub, methodParams, reflection, paramKey, methodParam, paramsCache, className, realClasses, boundModel, cache, handlerClass, reflectionClass, paramsKeys, paramValue, _0, *_1, _2, _5$$7, _6$$5, _7$$5, _11$$15, _12$$15, _14$$22, _15$$20, _16$$20, _18$$30, _19$$30;
	zval *this_ptr = getThis();
	...
	if (Z_TYPE_P(methodName) != IS_NULL) {
		...
		ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 312, handler, methodName);
		zephir_check_call_status();
	} else {
		object_init_ex(&reflection, zephir_get_internal_ce(SL("reflectionfunction")));
		ZEPHIR_CALL_METHOD(NULL, &reflection, "__construct", NULL, 78, handler);
		zephir_check_call_status();
	}
	zephir_read_property(&_0, this_ptr, SL("cache"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CPY_WRT(&cache, &_0);
	ZEPHIR_CALL_METHOD(&methodParams, &reflection, "getparameters", NULL, 79);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&paramsKeys);
	zephir_array_keys(&paramsKeys, &params);
	zephir_is_iterable(&methodParams, 0, "phalcon/mvc/model/binder.zep", 209);
	if (Z_TYPE_P(&methodParams) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&methodParams), _3, _4, _1)
		{
			...
			if (!(zephir_array_isset(&params, &paramKey))) {
				zephir_array_fetch(&_5$$7, &paramsKeys, &paramKey, PH_NOISY | PH_READONLY, "phalcon/mvc/model/binder.zep", 169);
				ZEPHIR_CPY_WRT(&paramKey, &_5$$7);
			}
			ZEPHIR_INIT_NVAR(&boundModel);
			ZVAL_NULL(&boundModel);
			ZEPHIR_OBS_NVAR(&paramValue);
			zephir_array_fetch(&paramValue, &params, &paramKey, PH_NOISY, "phalcon/mvc/model/binder.zep", 172);
			ZEPHIR_INIT_NVAR(&_6$$5);
			ZVAL_STRING(&_6$$5, "Phalcon\\Mvc\\Model");
			ZEPHIR_CALL_FUNCTION(&_7$$5, "is_subclass_of", &_8, 313, &className, &_6$$5);
			zephir_check_call_status();
			if (ZEPHIR_IS_STRING(&className, "Phalcon\\Mvc\\Model")) {
				...
				if (Z_TYPE_P(&realClasses) == IS_ARRAY) {
					...
					if (zephir_array_isset_fetch(&className, &realClasses, &paramKey, 0)) {
						ZEPHIR_CALL_METHOD(&boundModel, this_ptr, "findboundmodel", &_10, 0, &paramValue, &className);
						zephir_check_call_status();
					} else {
						ZEPHIR_INIT_NVAR(&_11$$15);
						object_init_ex(&_11$$15, phalcon_mvc_model_exception_ce);
						ZEPHIR_INIT_NVAR(&_12$$15);
						ZEPHIR_CONCAT_SVS(&_12$$15, "You should provide model class name for ", &paramKey, " parameter");
						ZEPHIR_CALL_METHOD(NULL, &_11$$15, "__construct", &_13, 4, &_12$$15);
						zephir_check_call_status();
						zephir_throw_exception_debug(&_11$$15, "phalcon/mvc/model/binder.zep", 189);
						ZEPHIR_MM_RESTORE();
						return;
					}
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_check_call_status();
				if (!(zephir_array_isset(&params, &paramKey))) {
					zephir_array_fetch(&_14$$22, &paramsKeys, &paramKey, PH_NOISY | PH_READONLY, "phalcon/mvc/model/binder.zep", 169);
					ZEPHIR_CPY_WRT(&paramKey, &_14$$22);
				}
				ZEPHIR_INIT_NVAR(&boundModel);
				ZVAL_NULL(&boundModel);
				ZEPHIR_OBS_NVAR(&paramValue);
				...
				ZVAL_STRING(&_15$$20, "Phalcon\\Mvc\\Model");
				ZEPHIR_CALL_FUNCTION(&_16$$20, "is_subclass_of", &_8, 313, &className, &_15$$20);
				zephir_check_call_status();
				if (ZEPHIR_IS_STRING(&className, "Phalcon\\Mvc\\Model")) {
					if (Z_TYPE_P(&realClasses) == IS_NULL) {
						if (zephir_instance_of_ev(handler, phalcon_mvc_controller_bindmodelinterface_ce)) {
							ZEPHIR_INIT_NVAR(&handlerClass);
							zephir_get_class(&handlerClass, handler, 0);
							_17$$25 = zephir_fetch_class(&handlerClass);
							ZEPHIR_CALL_CE_STATIC(&realClasses, _17$$25, "getmodelname", NULL, 0);
							zephir_check_call_status();
						} else if (zephir_instance_of_ev(handler, phalcon_mvc_model_binder_bindableinterface_ce)) {
							ZEPHIR_CALL_METHOD(&realClasses, handler, "getmodelname", NULL, 0);
							zephir_check_call_status();
					...
					if (Z_TYPE_P(&realClasses) == IS_ARRAY) {
						...
						if (zephir_array_isset_fetch(&className, &realClasses, &paramKey, 0)) {
							...
							ZEPHIR_INIT_NVAR(&_18$$30);
							object_init_ex(&_18$$30, phalcon_mvc_model_exception_ce);
							ZEPHIR_INIT_NVAR(&_19$$30);
							ZEPHIR_CONCAT_SVS(&_19$$30, "You should provide model class name for ", &paramKey, " parameter");
							ZEPHIR_CALL_METHOD(NULL, &_18$$30, "__construct", &_13, 4, &_19$$30);
							zephir_check_call_status();
							zephir_throw_exception_debug(&_18$$30, "phalcon/mvc/model/binder.zep", 189);
							ZEPHIR_MM_RESTORE();
							return;
						}
					} else if (Z_TYPE_P(&realClasses) == IS_STRING) {
