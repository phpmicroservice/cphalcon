...
ZEPHIR_INIT_CLASS(Phalcon_Mvc_Model_Criteria) {
	...
	zend_declare_property_null(phalcon_mvc_model_criteria_ce, SL("_params"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, bind) {
	...
	if (merge) {
		...
		if (zephir_array_isset_string(&_0$$3, SL("bind"))) {
			...
			ZEPHIR_OBS_VAR(&bind);
			zephir_array_fetch_string(&bind, &_1$$4, SL("bind"), PH_NOISY, "phalcon/mvc/model/criteria.zep", 108);
		} else {
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, inWhere) {
	...
	array_init(&bindKeys);
	zephir_is_iterable(&values, 0, "phalcon/mvc/model/criteria.zep", 420);
	...
	ZEPHIR_INIT_NVAR(&value);
	ZEPHIR_INIT_VAR(&_3);
	zephir_fast_join_str(&_3, SL(", "), &bindKeys);
	ZEPHIR_INIT_VAR(&_4);
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, notInWhere) {
	...
	ZEPHIR_INIT_VAR(&bindKeys);
	array_init(&bindKeys);
	zephir_is_iterable(&values, 0, "phalcon/mvc/model/criteria.zep", 457);
	if (Z_TYPE_P(&values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&values), _0)
		{
			...
			zephir_array_append(&bindKeys, &_2$$3, PH_SEPARATE, "phalcon/mvc/model/criteria.zep", 447);
			zephir_array_update_zval(&bindParams, &key, &value, PH_COPY | PH_SEPARATE);
			ZEPHIR_SEPARATE(&hiddenParam);
			zephir_increment(&hiddenParam);
		} ZEND_HASH_FOREACH_END();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_array_append(&bindKeys, &_3$$4, PH_SEPARATE, "phalcon/mvc/model/criteria.zep", 447);
				zephir_array_update_zval(&bindParams, &key, &value, PH_COPY | PH_SEPARATE);
				ZEPHIR_SEPARATE(&hiddenParam);
				zephir_increment(&hiddenParam);
			ZEPHIR_CALL_METHOD(NULL, &values, "next", NULL, 0);
	...
	ZEPHIR_INIT_NVAR(&value);
	ZEPHIR_INIT_VAR(&_4);
	zephir_fast_join_str(&_4, SL(", "), &bindKeys);
	ZEPHIR_INIT_VAR(&_5);
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, fromInput) {
	...
	if (zephir_fast_count_int(&data)) {
		...
		array_init(&bind);
		zephir_is_iterable(&data, 0, "phalcon/mvc/model/criteria.zep", 713);
		if (Z_TYPE_P(&data) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _6$$3, _7$$3, _4$$3)
			{
				...
				if (_8$$4) {
					_8$$4 = ((zephir_fast_count_int(&columnMap)) ? 1 : 0);
				}
				if (_8$$4) {
					ZEPHIR_OBS_NVAR(&attribute);
					zephir_array_fetch(&attribute, &columnMap, &field, PH_NOISY, "phalcon/mvc/model/criteria.zep", 690);
				} else {
				...
				if (zephir_array_isset_fetch(&type, &dataTypes, &attribute, 0)) {
					...
					if (_9$$7) {
						if (ZEPHIR_IS_LONG(&type, 2)) {
							...
							zephir_array_append(&conditions, &_10$$9, PH_SEPARATE, "phalcon/mvc/model/criteria.zep", 702);
						...
						zephir_array_append(&conditions, &_12$$8, PH_SEPARATE, "phalcon/mvc/model/criteria.zep", 709);
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					if (_13$$10) {
						_13$$10 = ((zephir_fast_count_int(&columnMap)) ? 1 : 0);
					}
					if (_13$$10) {
						ZEPHIR_OBS_NVAR(&attribute);
						zephir_array_fetch(&attribute, &columnMap, &field, PH_NOISY, "phalcon/mvc/model/criteria.zep", 690);
					...
					if (zephir_array_isset_fetch(&type, &dataTypes, &attribute, 0)) {
						...
						if (_14$$13) {
							if (ZEPHIR_IS_LONG(&type, 2)) {
								...
								zephir_array_append(&conditions, &_15$$15, PH_SEPARATE, "phalcon/mvc/model/criteria.zep", 702);
							...
							zephir_array_append(&conditions, &_17$$14, PH_SEPARATE, "phalcon/mvc/model/criteria.zep", 709);
							zephir_array_update_zval(&bind, &field, &value, PH_COPY | PH_SEPARATE);
						}
					}
