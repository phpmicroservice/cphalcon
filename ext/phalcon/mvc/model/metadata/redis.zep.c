...
ZEPHIR_INIT_CLASS(Phalcon_Mvc_Model_MetaData_Redis) {
	...
	zend_declare_property_long(phalcon_mvc_model_metadata_redis_ce, SL("_ttl"), 172800, ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_mvc_model_metadata_redis_ce, SL("_redis"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_mvc_model_metadata_redis_ce, SL("_metaData"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Redis, __construct) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_5, "__construct", NULL, 319, &_6);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &_4, "__construct", NULL, 322, &_5, options);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Redis, write) {
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *key_param = NULL, *data, data_sub, _0;
	zval key;
	zval *this_ptr = getThis();
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Redis, reset) {
	...
	if (Z_TYPE_P(&meta) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&meta) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&meta), _3$$3, _4$$3, _1$$3)
			{
				...
				zephir_read_property(&_5$$4, this_ptr, SL("_redis"), PH_NOISY_CC | PH_READONLY);
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&realKey);
					ZEPHIR_CONCAT_SV(&realKey, "meta-", &key);
					zephir_read_property(&_6$$5, this_ptr, SL("_redis"), PH_NOISY_CC | PH_READONLY);
	...
	ZEPHIR_CALL_PARENT(NULL, phalcon_mvc_model_metadata_redis_ce, getThis(), "reset", &_7, 0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
