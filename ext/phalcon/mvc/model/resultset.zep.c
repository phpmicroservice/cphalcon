...
ZEPHIR_INIT_CLASS(Phalcon_Mvc_Model_Resultset) {
	 ...
	 */
	zend_declare_property_bool(phalcon_mvc_model_resultset_ce, SL("_result"), 0, ZEND_ACC_PROTECTED);
	...
	zend_declare_property_long(phalcon_mvc_model_resultset_ce, SL("_pointer"), 0, ZEND_ACC_PROTECTED);
	...
	zend_declare_property_null(phalcon_mvc_model_resultset_ce, SL("_row"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Mvc_Model_Resultset, seek) {
	...
	if (_1) {
		...
		while (1) {
			...
			RETURN_ON_FAILURE(zephir_property_incr(this_ptr, SL("_pointer")));
		}
		zephir_update_property_zval(this_ptr, SL("_pointer"), position);
		zephir_update_property_zval(this_ptr, SL("_activeRow"), &__$null);
	}
...
PHP_METHOD(Phalcon_Mvc_Model_Resultset, offsetGet) {
	...
	if (ZEPHIR_LT(index, &_0)) {
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "seek", NULL, 72, index);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Resultset, update) {
	...
	while (1) {
		...
		zephir_check_call_status();
		if (transaction == 0) {
			if (!((zephir_method_exists_ex(&record, ZEND_STRL("getwriteconnection")) == SUCCESS))) {
