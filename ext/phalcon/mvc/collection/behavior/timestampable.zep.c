...
PHP_METHOD(Phalcon_Mvc_Collection_Behavior_Timestampable, notify) {
	...
	if (Z_TYPE_P(&options) == IS_ARRAY) {
		...
		if (zephir_array_isset_string_fetch(&format, &options, SL("format"), 0)) {
			ZEPHIR_CALL_FUNCTION(&timestamp, "date", NULL, 287, &format);
			zephir_check_call_status();
		} else {
			...
			if (zephir_array_isset_string_fetch(&generator, &options, SL("generator"), 0)) {
				if (Z_TYPE_P(&generator) == IS_OBJECT) {
					if (zephir_instance_of_ev(&generator, zend_ce_closure)) {
		...
		if (Z_TYPE_P(&field) == IS_ARRAY) {
			zephir_is_iterable(&field, 0, "phalcon/mvc/collection/behavior/timestampable.zep", 94);
			if (Z_TYPE_P(&field) == IS_ARRAY) {
