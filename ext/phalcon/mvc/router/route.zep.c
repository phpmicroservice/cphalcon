...
ZEPHIR_INIT_CLASS(Phalcon_Mvc_Router_Route) {
	...
	zend_declare_property_null(phalcon_mvc_router_route_ce, SL("_match"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_mvc_router_route_ce, SL("_group"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_mvc_router_route_ce, SL("_uniqueId"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC);
...
PHP_METHOD(Phalcon_Mvc_Router_Route, __construct) {
	...
	zephir_update_static_property_ce(phalcon_mvc_router_route_ce, ZEND_STRL("_uniqueId"), &_0);
	ZEPHIR_MM_RESTORE();
							...
							if (!(notValid)) {
								...
								if (_25$$18) {
									...
									if (foundPattern != 2) {
										...
										zephir_concat_self(&route, &_27$$26);
									} else {
								...
								zephir_concat_self(&route, &_30$$29);
							}
							continue;
						}
					}
...
PHP_METHOD(Phalcon_Mvc_Router_Route, reConfigure) {
	...
	if (!(zephir_start_with_str(&pattern, SL("#")))) {
		if (zephir_memnstr_str(&pattern, SL("{"), "phalcon/mvc/router/route.zep", 309)) {
			...
			zephir_array_fetch_long(&pcrePattern, &extracted, 0, PH_NOISY, "phalcon/mvc/router/route.zep", 314);
			ZEPHIR_INIT_VAR(&_0$$4);
			zephir_array_fetch_long(&_1$$4, &extracted, 1, PH_NOISY | PH_READONLY, "phalcon/mvc/router/route.zep", 315);
		...
		if (Z_TYPE_P(paths) == IS_STRING) {
			...
			do {
				...
				if (_0$$4 == 3) {
					...
					zephir_array_fetch_long(&moduleName, &parts, 0, PH_NOISY, "phalcon/mvc/router/route.zep", 367);
					ZEPHIR_OBS_NVAR(&controllerName);
					zephir_array_fetch_long(&controllerName, &parts, 1, PH_NOISY, "phalcon/mvc/router/route.zep", 368);
					ZEPHIR_OBS_NVAR(&actionName);
					zephir_array_fetch_long(&actionName, &parts, 2, PH_NOISY, "phalcon/mvc/router/route.zep", 369);
					break;
				}
				if (_0$$4 == 2) {
					ZEPHIR_OBS_NVAR(&controllerName);
					zephir_array_fetch_long(&controllerName, &parts, 0, PH_NOISY, "phalcon/mvc/router/route.zep", 373);
					ZEPHIR_OBS_NVAR(&actionName);
					zephir_array_fetch_long(&actionName, &parts, 1, PH_NOISY, "phalcon/mvc/router/route.zep", 374);
					break;
				}
				if (_0$$4 == 1) {
					ZEPHIR_OBS_NVAR(&controllerName);
					zephir_array_fetch_long(&controllerName, &parts, 0, PH_NOISY, "phalcon/mvc/router/route.zep", 378);
					break;
				}
			} while(0);
...
PHP_METHOD(Phalcon_Mvc_Router_Route, getReversedPaths) {
	...
	zephir_is_iterable(&_0, 0, "phalcon/mvc/router/route.zep", 566);
...
PHP_METHOD(Phalcon_Mvc_Router_Route, reset) {
	...
	zephir_update_static_property_ce(phalcon_mvc_router_route_ce, ZEND_STRL("_uniqueId"), &__$null);
