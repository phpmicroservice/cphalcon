...
ZEPHIR_INIT_CLASS(Phalcon_Mvc_View_Engine_Volt) {
	...
	zend_declare_property_null(phalcon_mvc_view_engine_volt_ce, SL("_macros"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, getCompiler) {
	...
	if (Z_TYPE_P(&compiler) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &compiler, "__construct", NULL, 368, &_0$$3);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, render) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zend_bool mustClean;
	zval *templatePath_param = NULL, *params, params_sub, *mustClean_param = NULL, compiler, compiledTemplatePath, key, value, *_0$$4, _1$$4, _4$$5, _5$$6, _6$$7, _7$$7;
	zval templatePath;
	zval *this_ptr = getThis();
	...
	if (mustClean) {
		...
		ZEPHIR_CALL_FUNCTION(&_7$$7, "ob_get_contents", NULL, 128);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, isIncluded) {
	...
	if (Z_TYPE_P(haystack) == IS_STRING) {
		if ((zephir_function_exists_ex(ZEND_STRL("mb_strpos")) == SUCCESS)) {
			ZEPHIR_CALL_FUNCTION(&_0$$5, "mb_strpos", NULL, 371, haystack, needle);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, convertEncoding) {
	...
	if ((zephir_function_exists_ex(ZEND_STRL("iconv")) == SUCCESS)) {
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, slice) {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("array_slice", NULL, 375, value, &_5$$9, &length);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, callMacro) {
	...
	if (!(zephir_array_isset_fetch(&macro, &_0, &name, 0))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "phalcon/mvc/view/engine/volt.zep", 287);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_RETURN_CALL_FUNCTION("call_user_func", NULL, 281, &macro, &arguments);
	zephir_check_call_status();
	RETURN_MM();
