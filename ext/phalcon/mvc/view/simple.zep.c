...
PHP_METHOD(Phalcon_Mvc_View_Simple, _loadTemplateEngines) {
	...
	zend_string *_4$$5;
	zend_ulong _3$$5;
	zval engines, dependencyInjector, registeredEngines, arguments, extension, engineService, engineObject, _0$$4, *_1$$5, _2$$5, _6$$13, _7$$13, _10$$20, _11$$20;
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&engines)) {
		...
		if (Z_TYPE_P(&registeredEngines) != IS_ARRAY) {
			...
			zephir_is_iterable(&registeredEngines, 0, "phalcon/mvc/view/simple.zep", 191);
			if (Z_TYPE_P(&registeredEngines) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&registeredEngines), _3$$5, _4$$5, _1$$5)
				{
					...
					if (Z_TYPE_P(&engineService) == IS_OBJECT) {
						...
						if (Z_TYPE_P(&engineService) == IS_STRING) {
							ZEPHIR_CALL_METHOD(&engineObject, &dependencyInjector, "getshared", &_5, 0, &engineService, &arguments);
							zephir_check_call_status();
						} else {
							ZEPHIR_INIT_NVAR(&_6$$13);
							object_init_ex(&_6$$13, phalcon_mvc_view_exception_ce);
							ZEPHIR_INIT_NVAR(&_7$$13);
							ZEPHIR_CONCAT_SV(&_7$$13, "Invalid template engine registration for extension: ", &extension);
							ZEPHIR_CALL_METHOD(NULL, &_6$$13, "__construct", &_8, 4, &_7$$13);
							zephir_check_call_status();
							zephir_throw_exception_debug(&_6$$13, "phalcon/mvc/view/simple.zep", 185);
							ZEPHIR_MM_RESTORE();
							return;
						}
					}
				...
				while (1) {
					...
					zephir_check_call_status();
						if (Z_TYPE_P(&engineService) == IS_OBJECT) {
							...
							if (Z_TYPE_P(&engineService) == IS_STRING) {
								ZEPHIR_CALL_METHOD(&engineObject, &dependencyInjector, "getshared", &_9, 0, &engineService, &arguments);
								zephir_check_call_status();
							} else {
								ZEPHIR_INIT_NVAR(&_10$$20);
								object_init_ex(&_10$$20, phalcon_mvc_view_exception_ce);
								ZEPHIR_INIT_NVAR(&_11$$20);
								ZEPHIR_CONCAT_SV(&_11$$20, "Invalid template engine registration for extension: ", &extension);
								ZEPHIR_CALL_METHOD(NULL, &_10$$20, "__construct", &_8, 4, &_11$$20);
								zephir_check_call_status();
								zephir_throw_exception_debug(&_10$$20, "phalcon/mvc/view/simple.zep", 185);
								ZEPHIR_MM_RESTORE();
								return;
							}
						}
...
PHP_METHOD(Phalcon_Mvc_View_Simple, _internalRender) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&engines, 0, "phalcon/mvc/view/simple.zep", 282);
	if (Z_TYPE_P(&engines) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&engines), _5, _6, _3)
		{
			...
			if ((zephir_file_exists(&_7$$6) == SUCCESS)) {
				...
				_11$$8 = _8$$8;
				if (_11$$8) {
					_11$$8 = (zephir_file_exists(&viewsDirPath) == SUCCESS);
				}
				if (_11$$8) {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if ((zephir_file_exists(&_18$$15) == SUCCESS)) {
					ZEPHIR_INIT_NVAR(&viewEnginePath);
					ZEPHIR_CONCAT_VV(&viewEnginePath, &viewsDirPath, &extension);
				} else {
					...
					_22$$17 = _19$$17;
					if (_22$$17) {
						_22$$17 = (zephir_file_exists(&viewsDirPath) == SUCCESS);
					}
					if (_22$$17) {
						ZEPHIR_CPY_WRT(&viewEnginePath, &viewsDirPath);
					} else {
	...
	if (notExists == 1) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_29$$24, "phalcon/mvc/view/simple.zep", 283);
		ZEPHIR_MM_RESTORE();
		return;
	}
	if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
