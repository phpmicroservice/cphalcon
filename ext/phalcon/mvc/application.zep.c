...
ZEPHIR_INIT_CLASS(Phalcon_Mvc_Application) {
	...
	zend_declare_property_bool(phalcon_mvc_application_ce, SL("_sendHeaders"), 1, ZEND_ACC_PROTECTED);

	zend_declare_property_bool(phalcon_mvc_application_ce, SL("_sendCookies"), 1, ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Mvc_Application, handle) {
	...
	if (Z_TYPE_P(&matchedRoute) == IS_OBJECT) {
		...
		if (Z_TYPE_P(&match) != IS_NULL) {
			...
			if (Z_TYPE_P(&possibleResponse) == IS_OBJECT) {
				if (zephir_instance_of_ev(&possibleResponse, phalcon_http_responseinterface_ce)) {
	...
	if (zephir_is_true(&moduleName)) {
		...
		if (Z_TYPE_P(&module) == IS_ARRAY) {
			...
			if (zephir_array_isset_string_fetch(&path, &module, SL("path"), 0)) {
				if (!(zephir_class_exists(&className, zephir_is_true(&__$false) ))) {
					if (!((zephir_file_exists(&path) == SUCCESS))) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_13$$21, "phalcon/mvc/application.zep", 239);
						ZEPHIR_MM_RESTORE();
						return;
					}
			...
			if (!(zephir_instance_of_ev(&module, zend_ce_closure))) {
			...
			zephir_create_array(&_16$$22, 1, 0);
	...
	if (_26) {
		...
		if (Z_TYPE_P(&possibleResponse) == IS_STRING) {
			...
			if (_31$$32) {
				_31$$32 = (zephir_instance_of_ev(&possibleResponse, phalcon_http_responseinterface_ce));
			}
