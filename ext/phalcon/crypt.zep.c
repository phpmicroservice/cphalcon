...
ZEPHIR_INIT_CLASS(Phalcon_Crypt) {
	...
	zend_declare_property_string(phalcon_crypt_ce, SL("_cipher"), "aes-256-cfb", ZEND_ACC_PROTECTED);
	 ...
	 * @var bool
	 */
	zend_declare_property_bool(phalcon_crypt_ce, SL("useSigning"), 0, ZEND_ACC_PROTECTED);
	...
	if (_0) {
		...
		do {
			if (paddingType == 1) {
				...
				ZEPHIR_CALL_FUNCTION(&_3$$5, "str_repeat", NULL, 139, &_2$$5, &_1$$5);
				zephir_check_call_status();
				ZVAL_LONG(&_1$$5, paddingSize);
				ZEPHIR_CALL_FUNCTION(&_4$$5, "chr", NULL, 138, &_1$$5);
				zephir_check_call_status();
			...
			if (paddingType == 2) {
				...
				ZEPHIR_CALL_FUNCTION(&_6$$6, "chr", NULL, 138, &_5$$6);
				zephir_check_call_status();
				ZVAL_LONG(&_5$$6, paddingSize);
			...
			if (paddingType == 3) {
				...
				if (_8$$7 <= _9$$7) {
					while (1) {
						...
						zephir_check_call_status();
						ZEPHIR_CALL_FUNCTION(&_12$$8, "chr", NULL, 138, &_10$$8);
						zephir_check_call_status();
						zephir_concat_self(&padding, &_12$$8);
					}
				}
				ZVAL_LONG(&_13$$7, paddingSize);
				ZEPHIR_CALL_FUNCTION(&_14$$7, "chr", NULL, 138, &_13$$7);
				zephir_check_call_status();
			...
			if (paddingType == 4) {
				...
				ZEPHIR_CALL_FUNCTION(&_16$$9, "chr", NULL, 138, &_15$$9);
				zephir_check_call_status();
				ZVAL_LONG(&_15$$9, 0);
				ZEPHIR_CALL_FUNCTION(&_17$$9, "chr", NULL, 138, &_15$$9);
				zephir_check_call_status();
				ZVAL_LONG(&_15$$9, (paddingSize - 1));
				ZEPHIR_CALL_FUNCTION(&_18$$9, "str_repeat", NULL, 139, &_17$$9, &_15$$9);
				zephir_check_call_status();
				ZEPHIR_INIT_NVAR(&padding);
			...
			if (paddingType == 5) {
				ZVAL_LONG(&_19$$10, 0);
				ZEPHIR_CALL_FUNCTION(&_20$$10, "chr", NULL, 138, &_19$$10);
				zephir_check_call_status();
		...
		do {
			if (paddingType == 1) {
				...
				if (ord <= blockSize) {
					...
					ZEPHIR_CALL_FUNCTION(&_8$$5, "str_repeat", NULL, 139, &_7$$5, &_6$$5);
					zephir_check_call_status();
					ZEPHIR_INIT_VAR(&padding);
			...
			if (paddingType == 2) {
				...
				ZEPHIR_CALL_FUNCTION(&_12$$7, "ord", NULL, 140, &last);
				zephir_check_call_status();
				ord = zephir_get_intval(&_12$$7);
				if (ord <= blockSize) {
					paddingSize = ord;
					ZVAL_LONG(&_13$$8, paddingSize);
					ZEPHIR_CALL_FUNCTION(&_14$$8, "chr", NULL, 138, &_13$$8);
					zephir_check_call_status();
					ZVAL_LONG(&_13$$8, paddingSize);
					ZEPHIR_CALL_FUNCTION(&padding, "str_repeat", NULL, 139, &_14$$8, &_13$$8);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Crypt, encrypt) {
	...
	if (EXPECTED(ZEPHIR_GT_LONG(&ivLength, 0))) {
		...
		ZVAL_STRING(&_5$$7, "");
		ZEPHIR_CALL_FUNCTION(&_6$$7, "str_ireplace", NULL, 142, &_4$$7, &_5$$7, &cipher);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&blockSize, this_ptr, "getivlength", NULL, 0, &_6$$7);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_FUNCTION(&iv, "openssl_random_pseudo_bytes", NULL, 143, &ivLength);
	zephir_check_call_status();
	...
	if (zephir_is_true(&_3)) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&digest$$10, "hash_hmac", NULL, 145, &hashAlgo$$10, &padded, &encryptKey, &__$true);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Crypt, decrypt) {
	...
	ZEPHIR_INIT_VAR(&_7);
	ZVAL_STRING(&_7, "8bit");
	ZEPHIR_CALL_FUNCTION(&iv, "mb_substr", NULL, 146, &text, &_3, &ivLength, &_7);
	zephir_check_call_status();
	zephir_read_property(&_3, this_ptr, SL("useSigning"), PH_NOISY_CC | PH_READONLY);
	if (zephir_is_true(&_3)) {
		...
		ZVAL_STRING(&_8$$8, "");
		ZEPHIR_CALL_FUNCTION(&_9$$8, "hash", NULL, 147, &hashAlgo, &_8$$8, &__$true);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&hashLength);
		ZVAL_LONG(&hashLength, zephir_fast_strlen_ev(&_9$$8));
		ZEPHIR_INIT_NVAR(&_8$$8);
		ZVAL_STRING(&_8$$8, "8bit");
		ZEPHIR_CALL_FUNCTION(&hash, "mb_substr", NULL, 146, &text, &ivLength, &hashLength, &_8$$8);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_10$$8);
		zephir_add_function(&_10$$8, &ivLength, &hashLength);
		ZEPHIR_INIT_NVAR(&_8$$8);
		ZVAL_STRING(&_8$$8, "8bit");
		ZEPHIR_CALL_FUNCTION(&ciphertext, "mb_substr", NULL, 146, &text, &_10$$8, &__$null, &_8$$8);
		zephir_check_call_status();
		ZVAL_LONG(&_11$$8, 1);
		ZEPHIR_CALL_FUNCTION(&decrypted, "openssl_decrypt", NULL, 148, &ciphertext, &cipher, &decryptKey, &_11$$8, &iv);
		zephir_check_call_status();
	...
	zephir_check_call_status();
	ZVAL_LONG(&_16, 1);
	ZEPHIR_CALL_FUNCTION(&decrypted, "openssl_decrypt", NULL, 148, &ciphertext, &cipher, &decryptKey, &_16, &iv);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Crypt, encryptBase64) {
	...
	if (safe == 1) {
		...
		ZVAL_STRING(&_3$$3, "-_");
		ZEPHIR_CALL_FUNCTION(&_4$$3, "strtr", NULL, 22, &_1$$3, &_2$$3, &_3$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Crypt, decryptBase64) {
	...
	if (safe == 1) {
		...
		ZVAL_STRING(&_1$$3, "+/");
		ZEPHIR_CALL_FUNCTION(&_2$$3, "strtr", NULL, 22, &text, &_0$$3, &_1$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Crypt, getAvailableHashAlgos) {
	...
	if (EXPECTED((zephir_function_exists_ex(ZEND_STRL("hash_hmac_algos")) == SUCCESS))) {
		ZEPHIR_CALL_FUNCTION(&algos, "hash_hmac_algos", NULL, 149);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_FUNCTION(&algos, "hash_algos", NULL, 150);
		zephir_check_call_status();
	}
	RETURN_CCTOR(&algos);
...
PHP_METHOD(Phalcon_Crypt, assertHashAlgorithmAvailable) {
	...
	zephir_check_call_status();
	if (!(zephir_fast_in_array(&hashAlgo, &availableAlgorithms))) {
		ZEPHIR_INIT_VAR(&_0$$3);
		object_init_ex(&_0$$3, phalcon_crypt_exception_ce);
		ZEPHIR_INIT_VAR(&_1$$3);
		ZVAL_STRING(&_1$$3, "The hash algorithm \"%s\" is not supported on this system.");
		ZEPHIR_CALL_FUNCTION(&_2$$3, "sprintf", NULL, 151, &_1$$3, &hashAlgo);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 4, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$3, "phalcon/crypt.zep", 591);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Crypt, getIvLength) {
	...
	if (!((zephir_function_exists_ex(ZEND_STRL("openssl_cipher_iv_length")) == SUCCESS))) {
...
PHP_METHOD(Phalcon_Crypt, initializeAvailableCiphers) {
	...
	if (!((zephir_function_exists_ex(ZEND_STRL("openssl_get_cipher_methods")) == SUCCESS))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_crypt_exception_ce, "openssl extension is required", "phalcon/crypt.zep", 617);
		return;
	}
	ZEPHIR_CALL_FUNCTION(&_0, "openssl_get_cipher_methods", NULL, 153, &__$true);
	zephir_check_call_status();
