...
ZEPHIR_INIT_CLASS(Phalcon_Http_Request_File) {
	...
	zend_declare_property_null(phalcon_http_request_file_ce, SL("_realType"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Http_Request_File, getRealType) {
	...
	ZVAL_LONG(&_0, 16);
	ZEPHIR_CALL_FUNCTION(&finfo, "finfo_open", NULL, 223, &_0);
	zephir_check_call_status();
	if (Z_TYPE_P(&finfo) != IS_RESOURCE) {
		RETURN_MM_STRING("");
	}
	zephir_read_property(&_0, this_ptr, SL("_tmp"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 224, &finfo, &_0);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(NULL, "finfo_close", NULL, 225, &finfo);
	zephir_check_call_status();
	RETURN_CCTOR(&mime);
...
PHP_METHOD(Phalcon_Http_Request_File, isUploadedFile) {
	...
	if (_0) {
		ZEPHIR_CALL_FUNCTION(&_1, "is_uploaded_file", NULL, 226, &tmp);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request_File, moveTo) {
	...
	zephir_read_property(&_0, this_ptr, SL("_tmp"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_RETURN_CALL_FUNCTION("move_uploaded_file", NULL, 227, &_0, &destination);
	zephir_check_call_status();
	RETURN_MM();
