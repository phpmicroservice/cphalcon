...
ZEPHIR_INIT_CLASS(Phalcon_Session_Bag) {
	...
	zend_declare_property_null(phalcon_session_bag_ce, SL("_dependencyInjector"), ZEND_ACC_PROTECTED);
	...
	zend_class_implements(phalcon_session_bag_ce, 1, zend_ce_aggregate);
	zend_class_implements(phalcon_session_bag_ce, 1, zend_ce_arrayaccess);
	zend_class_implements(phalcon_session_bag_ce, 1, spl_ce_Countable);
	return SUCCESS;
...
PHP_METHOD(Phalcon_Session_Bag, get) {
	...
	if (zephir_array_isset_fetch(&value, &_1, &property, 1)) {
		RETURN_CTOR(&value);
	}
