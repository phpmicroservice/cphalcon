...
PHP_METHOD(Phalcon_Tag_Select, selectField) {
	...
	if (!(zephir_array_isset_long_fetch(&id, &params, 0, 0))) {
		zephir_array_fetch_string(&_0$$5, &params, SL("id"), PH_NOISY | PH_READONLY, "phalcon/tag/select.zep", 52);
	...
	if (Z_TYPE_P(&options) == IS_OBJECT) {
		...
		ZEPHIR_CALL_SELF(&_8$$24, "_optionsfromresultset", &_9, 420, &options, &using, &value, &_11$$24);
		zephir_check_call_status();
		zephir_concat_self(&code, &_8$$24);
	} else {
		if (Z_TYPE_P(&options) == IS_ARRAY) {
			...
			ZEPHIR_CALL_SELF(&_12$$26, "_optionsfromarray", &_13, 421, &options, &value, &_15$$26);
			zephir_check_call_status();
			zephir_concat_self(&code, &_12$$26);
		} else {
...
PHP_METHOD(Phalcon_Tag_Select, _optionsFromResultset) {
	...
	if (Z_TYPE_P(using) == IS_ARRAY) {
		if (zephir_fast_count_int(using) != 2) {
		...
		zephir_array_fetch_long(&usingZero, using, 0, PH_NOISY, "phalcon/tag/select.zep", 168);
		ZEPHIR_OBS_VAR(&usingOne);
		zephir_array_fetch_long(&usingOne, using, 1, PH_NOISY, "phalcon/tag/select.zep", 168);
	}
		...
		if (Z_TYPE_P(using) == IS_ARRAY) {
			if (Z_TYPE_P(&option) == IS_OBJECT) {
				...
				if (Z_TYPE_P(&option) == IS_ARRAY) {
					ZEPHIR_OBS_NVAR(&optionValue);
					zephir_array_fetch(&optionValue, &option, &usingZero, PH_NOISY, "phalcon/tag/select.zep", 187);
					ZEPHIR_OBS_NVAR(&optionText);
					zephir_array_fetch(&optionText, &option, &usingOne, PH_NOISY, "phalcon/tag/select.zep", 188);
				} else {
			...
			if (Z_TYPE_P(value) == IS_ARRAY) {
				...
				if (ZEPHIR_IS_IDENTICAL(&strOptionValue, &strValue)) {
					...
					zephir_concat_self(&code, &_11$$18);
				}
			}
		} else {
			if (Z_TYPE_P(using) == IS_OBJECT) {
				...
				zephir_check_call_status();
				zephir_concat_self(&code, &_12$$20);
			}
		}
	}
...
PHP_METHOD(Phalcon_Tag_Select, _optionsFromArray) {
	...
	zval _12$$8, _13$$8, _22$$16, _23$$16;
	zend_string *_3;
	zend_ulong _2;
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zephir_fcall_cache_entry *_4 = NULL, *_7 = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *data, data_sub, *value, value_sub, *closeOption, closeOption_sub, strValue, strOptionValue, code, optionValue, optionText, escaped, *_0, _1, _5$$4, _6$$4, _8$$4, _9$$4, _10$$6, _11$$7, _14$$9, _15$$10, _16$$12, _17$$12, _18$$12, _19$$12, _20$$14, _21$$15, _24$$17, _25$$18;
	zval *this_ptr = getThis();
	...
	if (Z_TYPE_P(data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(data), _2, _3, _0)
		{
			...
			ZEPHIR_CALL_FUNCTION(&escaped, "htmlspecialchars", &_4, 186, &optionValue);
			zephir_check_call_status();
			if (Z_TYPE_P(&optionText) == IS_ARRAY) {
				ZEPHIR_INIT_NVAR(&_5$$4);
				ZEPHIR_GET_CONSTANT(&_5$$4, "PHP_EOL");
				ZEPHIR_CALL_SELF(&_6$$4, "_optionsfromarray", &_7, 421, &optionText, value, closeOption);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (Z_TYPE_P(&optionText) == IS_ARRAY) {
					...
					ZEPHIR_CALL_SELF(&_17$$12, "_optionsfromarray", &_7, 421, &optionText, value, closeOption);
					zephir_check_call_status();
