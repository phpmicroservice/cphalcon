...
PHP_METHOD(Phalcon_Validation_Validator, getOption) {
	...
	if (Z_TYPE_P(&options) == IS_ARRAY) {
		if (zephir_array_isset_fetch(&value, &options, &key, 1)) {
...
PHP_METHOD(Phalcon_Validation_Validator, prepareLabel) {
	...
	if (Z_TYPE_P(&label) == IS_ARRAY) {
		zephir_array_fetch(&_1$$3, &label, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator.zep", 110);
...
PHP_METHOD(Phalcon_Validation_Validator, prepareMessage) {
	...
	if (Z_TYPE_P(&message) == IS_ARRAY) {
		zephir_array_fetch(&_0$$3, &message, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator.zep", 129);
...
PHP_METHOD(Phalcon_Validation_Validator, prepareCode) {
	...
	if (Z_TYPE_P(&code) == IS_ARRAY) {
		zephir_array_fetch(&_1$$3, &code, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator.zep", 148);
