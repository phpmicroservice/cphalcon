...
PHP_METHOD(Phalcon_Validation_Validator_Identical, validate) {
	...
	zval field;
	zval *validation, validation_sub, *field_param = NULL, message, label, replacePairs, value, accepted, valueOption, code, _0, _1, _2$$3, _3$$4, _4$$5, _5$$5, _6$$6, _7$$7, _8$$8, _9$$8, _10$$8;
	...
	if (zephir_is_true(&_0)) {
		...
		if (Z_TYPE_P(&accepted) == IS_ARRAY) {
			zephir_array_fetch(&_3$$4, &accepted, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/identical.zep", 82);
		...
		if (zephir_is_true(&_4$$5)) {
			...
			if (Z_TYPE_P(&valueOption) == IS_ARRAY) {
				zephir_array_fetch(&_7$$7, &valueOption, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/identical.zep", 89);
	...
	if (!(valid)) {
		...
		ZEPHIR_CALL_FUNCTION(&_9$$8, "strtr", NULL, 22, &message, &replacePairs);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_10$$8);
		ZVAL_STRING(&_10$$8, "Identical");
		ZEPHIR_CALL_METHOD(NULL, &_8$$8, "__construct", NULL, 434, &_9$$8, &field, &_10$$8, &code);
		zephir_check_call_status();
