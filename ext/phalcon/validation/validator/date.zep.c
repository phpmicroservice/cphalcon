...
PHP_METHOD(Phalcon_Validation_Validator_Date, validate) {
	...
	zval field;
	zval *validation, validation_sub, *field_param = NULL, value, format, label, message, replacePairs, code, _0, _2, _1$$3, _3$$5, _4$$5, _5$$5;
	...
	if (Z_TYPE_P(&format) == IS_ARRAY) {
		zephir_array_fetch(&_1$$3, &format, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/date.zep", 80);
	...
	ZEPHIR_CALL_METHOD(&_2, this_ptr, "checkdate", NULL, 440, &value, &format);
	zephir_check_call_status();
	if (!(zephir_is_true(&_2))) {
		ZEPHIR_CALL_METHOD(&label, this_ptr, "preparelabel", NULL, 0, validation, &field);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_3$$5);
		ZVAL_STRING(&_3$$5, "Date");
		...
		ZEPHIR_CALL_FUNCTION(&_4$$5, "strtr", NULL, 22, &message, &replacePairs);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_5$$5);
		ZVAL_STRING(&_5$$5, "Date");
		ZEPHIR_CALL_METHOD(NULL, &_3$$5, "__construct", NULL, 434, &_4$$5, &field, &_5$$5, &code);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation_Validator_Date, checkDate) {
	...
	zephir_check_call_status();
	zephir_array_fetch_string(&_2, &errors, SL("warning_count"), PH_NOISY | PH_READONLY, "phalcon/validation/validator/date.zep", 120);
	_3 = ZEPHIR_GT_LONG(&_2, 0);
	if (!(_3)) {
		zephir_array_fetch_string(&_4, &errors, SL("error_count"), PH_NOISY | PH_READONLY, "phalcon/validation/validator/date.zep", 120);
