...
PHP_METHOD(Phalcon_Validation_Validator_Confirmation, validate) {
	...
	if (Z_TYPE_P(&fieldWith) == IS_ARRAY) {
		zephir_array_fetch(&_1$$3, &fieldWith, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/confirmation.zep", 81);
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_2, this_ptr, "compare", NULL, 437, &value, &valueWith);
	zephir_check_call_status();
	if (!(zephir_is_true(&_2))) {
		...
		if (Z_TYPE_P(&labelWith) == IS_ARRAY) {
			zephir_array_fetch(&_4$$5, &labelWith, &fieldWith, PH_NOISY | PH_READONLY, "phalcon/validation/validator/confirmation.zep", 94);
		...
		ZEPHIR_CALL_METHOD(NULL, &_3$$4, "__construct", NULL, 434, &_5$$4, &field, &_6$$4, &code);
		zephir_check_call_status();
