...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, validate) {
	...
	if (!(zephir_is_true(&valid))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 434, &_1$$3, &field, &_2$$3, &code);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, verifyByLuhnAlgorithm) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "str_split", NULL, 69, number);
	zephir_check_call_status();
	zephir_get_arrval(&_1, &_0);
	ZEPHIR_CPY_WRT(&digits, &_1);
	ZEPHIR_INIT_VAR(&hash);
	ZVAL_STRING(&hash, "");
	ZEPHIR_CALL_FUNCTION(&_3, "array_reverse", NULL, 418, &digits);
	zephir_check_call_status();
	...
	ZEPHIR_INIT_NVAR(&digit);
	ZEPHIR_INIT_NVAR(&position);
	ZEPHIR_CALL_FUNCTION(&_10, "str_split", NULL, 69, &hash);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&result, "array_sum", NULL, 439, &_10);
	zephir_check_call_status();
