...
PHP_METHOD(Phalcon_Validation_Validator_ExclusionIn, validate) {
	...
	if (zephir_is_true(&_1)) {
		...
		if (Z_TYPE_P(&strict) == IS_ARRAY) {
			zephir_array_fetch(&_3$$7, &strict, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/exclusionin.zep", 105);
	...
	ZEPHIR_CALL_FUNCTION(&_4, "in_array", NULL, 362, &value, &domain, &strict);
	zephir_check_call_status();
	if (zephir_is_true(&_4)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_5$$9, "__construct", NULL, 434, &_6$$9, &field, &_7$$9, &code);
		zephir_check_call_status();
