...
PHP_METHOD(Phalcon_Validation_Validator_StringLength, validate) {
	...
	zval field;
	zval *validation, validation_sub, *field_param = NULL, isSetMin, isSetMax, value, length, message, minimum, maximum, label, replacePairs, code, _0, _2$$6, _3$$7, _4$$8, _5$$8, _6$$8, _7$$9, _8$$10, _9$$11, _10$$11, _11$$11, _12$$11;
	...
	if (zephir_is_true(&isSetMax)) {
		...
		if (Z_TYPE_P(&maximum) == IS_ARRAY) {
			zephir_array_fetch(&_3$$7, &maximum, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/stringlength.zep", 116);
		...
		if (ZEPHIR_GT(&length, &maximum)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_4$$8, "__construct", NULL, 434, &_6$$8, &field, &_5$$8, &code);
			zephir_check_call_status();
	...
	if (zephir_is_true(&isSetMin)) {
		...
		if (Z_TYPE_P(&minimum) == IS_ARRAY) {
			zephir_array_fetch(&_8$$10, &minimum, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/stringlength.zep", 142);
		...
		if (ZEPHIR_LT(&length, &minimum)) {
			...
			zephir_create_array(&_11$$11, 2, 0);
			...
			ZEPHIR_CALL_FUNCTION(&_12$$11, "strtr", NULL, 22, &message, &replacePairs);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&_10$$11);
			ZVAL_STRING(&_10$$11, "TooShort");
			ZEPHIR_CALL_METHOD(NULL, &_9$$11, "__construct", NULL, 434, &_12$$11, &field, &_10$$11, &code);
			zephir_check_call_status();
