...
ZEPHIR_INIT_CLASS(Phalcon_Cli_Router_Route) {
	...
	zend_declare_property_null(phalcon_cli_router_route_ce, SL("_pattern"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_cli_router_route_ce, SL("_compiledPattern"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_cli_router_route_ce, SL("_paths"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_cli_router_route_ce, SL("_converters"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_cli_router_route_ce, SL("_id"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_cli_router_route_ce, SL("_name"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_cli_router_route_ce, SL("_beforeMatch"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_cli_router_route_ce, SL("_delimiter"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_cli_router_route_ce, SL("_uniqueId"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC);

	zend_declare_property_null(phalcon_cli_router_route_ce, SL("_delimiterPath"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC);
...
PHP_METHOD(Phalcon_Cli_Router_Route, __construct) {
	...
	zephir_update_static_property_ce(phalcon_cli_router_route_ce, ZEND_STRL("_uniqueId"), &_0);
	ZEPHIR_MM_RESTORE();
						...
						if (bracketCount == 0) {
							...
							if (!(notValid)) {
								...
								if (_25$$18) {
									...
									zephir_concat_self(&route, &_29$$28);
...
PHP_METHOD(Phalcon_Cli_Router_Route, reConfigure) {
		...
		if (Z_TYPE_P(paths) == IS_STRING) {
			...
			do {
				...
				if (_0$$4 == 3) {
					...
					zephir_array_fetch_long(&moduleName, &parts, 0, PH_NOISY, "phalcon/cli/router/route.zep", 318);
					ZEPHIR_OBS_NVAR(&taskName);
					zephir_array_fetch_long(&taskName, &parts, 1, PH_NOISY, "phalcon/cli/router/route.zep", 319);
					ZEPHIR_OBS_NVAR(&actionName);
					zephir_array_fetch_long(&actionName, &parts, 2, PH_NOISY, "phalcon/cli/router/route.zep", 320);
					break;
				}
				if (_0$$4 == 2) {
					ZEPHIR_OBS_NVAR(&taskName);
					zephir_array_fetch_long(&taskName, &parts, 0, PH_NOISY, "phalcon/cli/router/route.zep", 324);
					ZEPHIR_OBS_NVAR(&actionName);
					zephir_array_fetch_long(&actionName, &parts, 1, PH_NOISY, "phalcon/cli/router/route.zep", 325);
					break;
				}
				if (_0$$4 == 1) {
					ZEPHIR_OBS_NVAR(&taskName);
					zephir_array_fetch_long(&taskName, &parts, 0, PH_NOISY, "phalcon/cli/router/route.zep", 329);
					break;
				}
			} while(0);
	...
	if (!(zephir_start_with_str(&pattern, SL("#")))) {
		if (zephir_memnstr_str(&pattern, SL("{"), "phalcon/cli/router/route.zep", 384)) {
			...
			zephir_array_fetch_long(&pcrePattern, &extracted, 0, PH_NOISY, "phalcon/cli/router/route.zep", 389);
			ZEPHIR_INIT_VAR(&_2$$18);
			zephir_array_fetch_long(&_3$$18, &extracted, 1, PH_NOISY | PH_READONLY, "phalcon/cli/router/route.zep", 390);
			zephir_fast_array_merge(&_2$$18, &routePaths, &_3$$18);
		...
		if (zephir_memnstr_str(&pattern, SL(":delimiter"), "phalcon/cli/router/route.zep", 401)) {
			...
			zephir_fast_str_replace(&_4$$21, &_6$$21, &_5$$21, &pattern);
...
PHP_METHOD(Phalcon_Cli_Router_Route, getReversedPaths) {
	...
	zephir_is_iterable(&_0, 0, "phalcon/cli/router/route.zep", 516);
...
PHP_METHOD(Phalcon_Cli_Router_Route, reset) {
	...
	zephir_update_static_property_ce(phalcon_cli_router_route_ce, ZEND_STRL("_uniqueId"), &__$null);
...
PHP_METHOD(Phalcon_Cli_Router_Route, delimiter) {
	...
	zephir_update_static_property_ce(phalcon_cli_router_route_ce, ZEND_STRL("_delimiterPath"), &delimiter);
	ZEPHIR_MM_RESTORE();
