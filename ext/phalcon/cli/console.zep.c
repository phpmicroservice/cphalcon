...
ZEPHIR_INIT_CLASS(Phalcon_Cli_Console) {
	...
	zend_declare_property_null(phalcon_cli_console_ce, SL("_arguments"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_cli_console_ce, SL("_options"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Cli_Console, addModules) {
	...
	ZEPHIR_OBS_COPY_OR_DUP(&modules, modules_param);
...
PHP_METHOD(Phalcon_Cli_Console, handle) {
	...
	_5 = !(zephir_fast_count_int(&arguments));
	if (_5) {
	...
	if (zephir_is_true(&moduleName)) {
		...
		if (!(zephir_array_isset(&modules, &moduleName))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_9$$12, "phalcon/cli/console.zep", 111);
			ZEPHIR_MM_RESTORE();
			return;
		}
		ZEPHIR_OBS_VAR(&module);
		zephir_array_fetch(&module, &modules, &moduleName, PH_NOISY, "phalcon/cli/console.zep", 114);
		...
		if (zephir_array_isset_string_fetch(&path, &module, SL("path"), 0)) {
			if (!((zephir_file_exists(&path) == SUCCESS))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_11$$15, "phalcon/cli/console.zep", 121);
				ZEPHIR_MM_RESTORE();
				return;
			}
...
PHP_METHOD(Phalcon_Cli_Console, setArgument) {
	...
	zephir_is_iterable(&arguments, 0, "phalcon/cli/console.zep", 201);
	if (Z_TYPE_P(&arguments) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&arguments), _1)
		{
			...
			if (Z_TYPE_P(&arg) == IS_STRING) {
				...
				ZVAL_STRING(&_3$$5, "--");
				ZVAL_LONG(&_4$$5, 2);
				ZEPHIR_CALL_FUNCTION(&_5$$5, "strncmp", &_6, 130, &arg, &_3$$5, &_4$$5);
				zephir_check_call_status();
				if (ZEPHIR_IS_LONG(&_5$$5, 0)) {
					...
					ZVAL_STRING(&_18$$9, "-");
					ZVAL_LONG(&_19$$9, 1);
					ZEPHIR_CALL_FUNCTION(&_20$$9, "strncmp", &_6, 130, &arg, &_18$$9, &_19$$9);
					zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				if (Z_TYPE_P(&arg) == IS_STRING) {
					...
					ZVAL_STRING(&_23$$14, "--");
					ZVAL_LONG(&_24$$14, 2);
					ZEPHIR_CALL_FUNCTION(&_25$$14, "strncmp", &_6, 130, &arg, &_23$$14, &_24$$14);
					zephir_check_call_status();
					if (ZEPHIR_IS_LONG(&_25$$14, 0)) {
						...
						ZVAL_STRING(&_37$$18, "-");
						ZVAL_LONG(&_38$$18, 1);
						ZEPHIR_CALL_FUNCTION(&_39$$18, "strncmp", &_6, 130, &arg, &_37$$18, &_38$$18);
						zephir_check_call_status();
	...
	if (str) {
		...
		zephir_update_property_zval(this_ptr, SL("_arguments"), &_42$$22);
	} else {
		if (zephir_fast_count_int(&args)) {
			ZEPHIR_MAKE_REF(&args);
			ZEPHIR_CALL_FUNCTION(&_45$$24, "array_shift", NULL, 14, &args);
			ZEPHIR_UNREF(&args);
			zephir_check_call_status();
		...
		if (zephir_fast_count_int(&args)) {
			ZEPHIR_MAKE_REF(&args);
			ZEPHIR_CALL_FUNCTION(&_46$$25, "array_shift", NULL, 14, &args);
			ZEPHIR_UNREF(&args);
			zephir_check_call_status();
