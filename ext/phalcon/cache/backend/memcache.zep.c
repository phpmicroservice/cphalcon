...
PHP_METHOD(Phalcon_Cache_Backend_Memcache, _connect) {
	...
	if (zephir_has_constructor(&memcache)) {
...
PHP_METHOD(Phalcon_Cache_Backend_Memcache, addServers) {
	...
	if (UNEXPECTED(Z_TYPE_P(host_param) != IS_STRING && Z_TYPE_P(host_param) != IS_NULL)) {
		zephir_throw_exception_string(spl_ce_InvalidArgumentException, SL("Parameter 'host' must be of the type string"));
		RETURN_MM_NULL();
	}
...
PHP_METHOD(Phalcon_Cache_Backend_Memcache, queryKeys) {
	...
	zephir_is_iterable(&keys, 1, "phalcon/cache/backend/memcache.zep", 365);
	if (Z_TYPE_P(&keys) == IS_ARRAY) {
...
PHP_METHOD(Phalcon_Cache_Backend_Memcache, exists) {
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long lifetime, ZEPHIR_LAST_CALL_STATUS;
	zval *keyName = NULL, keyName_sub, *lifetime_param = NULL, __$null, lastKey, memcache, prefix, _0$$5;
	zval *this_ptr = getThis();
...
PHP_METHOD(Phalcon_Cache_Backend_Memcache, flush) {
	...
	ZEPHIR_INIT_VAR(&_0);
	zephir_is_iterable(&keys, 0, "phalcon/cache/backend/memcache.zep", 492);
	if (Z_TYPE_P(&keys) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&keys), _3, _4, _1)
		{
			ZEPHIR_INIT_NVAR(&key);
