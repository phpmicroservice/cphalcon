...
PHP_METHOD(Phalcon_Cache_Backend_Mongo, _getCollection) {
	...
	if (Z_TYPE_P(&mongoCollection) != IS_OBJECT) {
		...
		if (zephir_array_isset_string_fetch(&mongo, &options, SL("mongo"), 0)) {
			...
			zephir_array_fetch_string(&server, &options, SL("server"), PH_NOISY, "phalcon/cache/backend/mongo.zep", 119);
			...
			if (zephir_has_constructor(&mongo)) {
				ZEPHIR_CALL_METHOD(NULL, &mongo, "__construct", NULL, 0, &server);
				zephir_check_call_status();
			}
		}
		ZEPHIR_OBS_VAR(&database);
		zephir_array_fetch_string(&database, &options, SL("db"), PH_NOISY, "phalcon/cache/backend/mongo.zep", 130);
		...
		zephir_array_fetch_string(&collection, &options, SL("collection"), PH_NOISY, "phalcon/cache/backend/mongo.zep", 138);
...
PHP_METHOD(Phalcon_Cache_Backend_Mongo, get) {
	...
	zval *keyName_param = NULL, *lifetime_param = NULL, frontend, prefixedKey, conditions, document, cachedContent, _0, _2, _3;
	zval keyName;
	zval *this_ptr = getThis();
...
PHP_METHOD(Phalcon_Cache_Backend_Mongo, save) {
	...
	if (Z_TYPE_P(&document) == IS_ARRAY) {
		...
		zephir_create_array(&_2$$14, 1, 0);
		ZEPHIR_OBS_VAR(&_3$$14);
		zephir_array_fetch_string(&_3$$14, &document, SL("_id"), PH_NOISY, "phalcon/cache/backend/mongo.zep", 243);
...
PHP_METHOD(Phalcon_Cache_Backend_Mongo, delete) {
	...
	zephir_check_call_status();
	if (zephir_safe_mod_long_long(zephir_get_intval(&_4), 100) == 0) {
...
PHP_METHOD(Phalcon_Cache_Backend_Mongo, queryKeys) {
	...
	zephir_check_call_status();
	_5 = zephir_get_iterator(&items);
	_5->funcs->rewind(_5);
	for (;_5->funcs->valid(_5) == SUCCESS && !EG(exception); _5->funcs->move_forward(_5)) {
		{
			ZEPHIR_ITERATOR_COPY(&item, _5);
		}
		zephir_is_iterable(&item, 0, "phalcon/cache/backend/mongo.zep", 317);
