...
ZEPHIR_INIT_CLASS(Phalcon_Cache_Backend_File) {
	 ...
	 * @var boolean
	 */
	zend_declare_property_bool(phalcon_cache_backend_file_ce, SL("_useSafeKey"), 0, ZEND_ACC_PRIVATE);
...
PHP_METHOD(Phalcon_Cache_Backend_File, __construct) {
	...
	if (zephir_array_isset_string_fetch(&prefix, &options, SL("prefix"), 0)) {
		...
		if (_1$$6) {
			...
			zephir_preg_match(&_4$$6, &_5$$6, &prefix, &_2$$6, 0, 0 , 0 );
...
PHP_METHOD(Phalcon_Cache_Backend_File, get) {
	...
	if ((zephir_file_exists(&cacheFile) == SUCCESS) == 1) {
		...
		if (ZEPHIR_LT_LONG(&_5$$4, (modifiedTime + ttl))) {
			...
			zephir_file_get_contents(&cachedContent, &cacheFile);
			if (ZEPHIR_IS_FALSE_IDENTICAL(&cachedContent)) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_6$$10, "phalcon/cache/backend/file.zep", 148);
				ZEPHIR_MM_RESTORE();
				return;
			}
...
PHP_METHOD(Phalcon_Cache_Backend_File, save) {
	...
	zephir_file_put_contents(&status, &cacheFile, &preparedContent);
	if (ZEPHIR_IS_FALSE_IDENTICAL(&status)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_3$$11, "phalcon/cache/backend/file.zep", 215);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Cache_Backend_File, exists) {
	...
	if (zephir_is_true(&lastKey)) {
		...
		zephir_array_fetch_string(&_2$$5, &_1$$5, SL("cacheDir"), PH_NOISY | PH_READONLY, "phalcon/cache/backend/file.zep", 315);
		ZEPHIR_INIT_VAR(&cacheFile);
...
PHP_METHOD(Phalcon_Cache_Backend_File, increment) {
	...
	zephir_update_property_zval(this_ptr, SL("_lastKey"), &prefixedKey);
	zephir_read_property(&_2, this_ptr, SL("_options"), PH_NOISY_CC | PH_READONLY);
	zephir_array_fetch_string(&_3, &_2, SL("cacheDir"), PH_NOISY | PH_READONLY, "phalcon/cache/backend/file.zep", 352);
	ZEPHIR_INIT_VAR(&cacheFile);
	...
	if ((zephir_file_exists(&cacheFile) == SUCCESS)) {
		...
		if (ZEPHIR_LT_LONG(&_5$$3, (modifiedTime + zephir_get_numberval(&ttl)))) {
			...
			if (ZEPHIR_IS_FALSE_IDENTICAL(&cachedContent)) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_6$$7, "phalcon/cache/backend/file.zep", 383);
				ZEPHIR_MM_RESTORE();
				return;
			}
			if (zephir_is_numeric(&cachedContent)) {
				...
				zephir_file_put_contents(&_8$$8, &cacheFile, &result);
...
PHP_METHOD(Phalcon_Cache_Backend_File, decrement) {
	...
	zephir_update_property_zval(this_ptr, SL("_lastKey"), &prefixedKey);
	zephir_read_property(&_2, this_ptr, SL("_options"), PH_NOISY_CC | PH_READONLY);
	zephir_array_fetch_string(&_3, &_2, SL("cacheDir"), PH_NOISY | PH_READONLY, "phalcon/cache/backend/file.zep", 412);
	ZEPHIR_INIT_VAR(&cacheFile);
	ZEPHIR_CONCAT_VV(&cacheFile, &_3, &prefixedKey);
	if ((zephir_file_exists(&cacheFile) == SUCCESS)) {
		ZEPHIR_OBS_VAR(&lifetime);
		...
		if (ZEPHIR_LT_LONG(&_6$$3, (modifiedTime + zephir_get_numberval(&ttl)))) {
			...
			if (ZEPHIR_IS_FALSE_IDENTICAL(&cachedContent)) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_7$$7, "phalcon/cache/backend/file.zep", 441);
				ZEPHIR_MM_RESTORE();
				return;
			}
			if (zephir_is_numeric(&cachedContent)) {
				...
				zephir_file_put_contents(&_9$$8, &cacheFile, &result);
...
PHP_METHOD(Phalcon_Cache_Backend_File, useSafeKey) {
	...
	zephir_fetch_params_without_memory_grow(1, 0, &useSafeKey_param);
