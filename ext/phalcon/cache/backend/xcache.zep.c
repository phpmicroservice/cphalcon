...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, phalcon_cache_backend_xcache_ce, getThis(), "__construct", &_1, 0, frontend, options);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, get) {
	...
	ZEPHIR_CALL_FUNCTION(&cachedContent, "xcache_get", NULL, 85, &prefixedKey);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, save) {
	...
	zval *keyName = NULL, keyName_sub, *content = NULL, content_sub, *lifetime = NULL, lifetime_sub, *stopBuffer_param = NULL, __$true, __$false, __$null, lastKey, frontend, cachedContent, preparedContent, tmp, tt1, success, isBuffering, options, keys, specialKey, _0$$4, _1$$17;
	zval *this_ptr = getThis();
	...
	ZEPHIR_CALL_FUNCTION(&success, "xcache_set", NULL, 86, &lastKey, &preparedContent, &tt1);
	zephir_check_call_status();
	...
	if (zephir_is_true(&success)) {
		...
		zephir_read_property(&_1$$17, this_ptr, SL("_options"), PH_NOISY_CC | PH_READONLY);
		if (!(zephir_array_isset_string_fetch(&specialKey, &_1$$17, SL("statsKey"), 0))) {
		...
		if (!ZEPHIR_IS_STRING(&specialKey, "")) {
			ZEPHIR_CALL_FUNCTION(&keys, "xcache_get", NULL, 85, &specialKey);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_FUNCTION(NULL, "xcache_set", NULL, 86, &specialKey, &keys);
			zephir_check_call_status();
		}
	}
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, queryKeys) {
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *prefix_param = NULL, options, prefixed, specialKey, keys, retval, key, realKey, _1, _2, *_3, _4, _7$$9, _8$$11;
	zval prefix, _0$$4;
	zval *this_ptr = getThis();
	...
	array_init(&retval);
	ZEPHIR_INIT_VAR(&_2);
	zephir_is_iterable(&keys, 0, "phalcon/cache/backend/xcache.zep", 272);
	if (Z_TYPE_P(&keys) == IS_ARRAY) {
		...
		while (1) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_2, &keys, "current", NULL, 0);
			zephir_check_call_status();
				if (zephir_start_with(&key, &prefixed, NULL)) {
					ZVAL_LONG(&_8$$11, 5);
					ZEPHIR_INIT_NVAR(&realKey);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, exists) {
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long lifetime, ZEPHIR_LAST_CALL_STATUS;
	zval *keyName = NULL, keyName_sub, *lifetime_param = NULL, __$null, lastKey, _0$$4;
	zval *this_ptr = getThis();
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, increment) {
	...
	if ((zephir_function_exists_ex(ZEND_STRL("xcache_inc")) == SUCCESS)) {
		ZVAL_LONG(&_1$$6, value);
		ZEPHIR_CALL_FUNCTION(&newVal, "xcache_inc", NULL, 118, &lastKey, &_1$$6);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_FUNCTION(&origVal, "xcache_get", NULL, 85, &lastKey);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&newVal);
		ZVAL_LONG(&newVal, (zephir_get_numberval(&origVal) - value));
		ZEPHIR_CALL_FUNCTION(NULL, "xcache_set", NULL, 86, &lastKey, &newVal);
		zephir_check_call_status();
	}
	RETURN_CCTOR(&newVal);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, decrement) {
	...
	if ((zephir_function_exists_ex(ZEND_STRL("xcache_dec")) == SUCCESS)) {
		ZVAL_LONG(&_1$$6, value);
		...
		ZEPHIR_INIT_NVAR(&newVal);
		ZVAL_LONG(&newVal, (zephir_get_numberval(&origVal) - value));
		ZEPHIR_CALL_FUNCTION(&success, "xcache_set", NULL, 86, &lastKey, &newVal);
		zephir_check_call_status();
	}
	RETURN_CCTOR(&newVal);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, flush) {
	...
	zend_string *_5$$5;
	zend_ulong _4$$5;
	zval options, specialKey, keys, key, _0, _1$$5, *_2$$5, _3$$5;
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	...
	if (Z_TYPE_P(&keys) == IS_ARRAY) {
		...
		zephir_is_iterable(&keys, 1, "phalcon/cache/backend/xcache.zep", 381);
		if (Z_TYPE_P(&keys) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&keys), _4$$5, _5$$5, _2$$5)
			{
				ZEPHIR_INIT_NVAR(&key);
				if (_5$$5 != NULL) { 
				...
				ZEPHIR_CALL_FUNCTION(NULL, "xcache_unset", &_6, 120, &key);
				zephir_check_call_status();
			} ZEND_HASH_FOREACH_END();
		} else {
			ZEPHIR_CALL_METHOD(NULL, &keys, "rewind", NULL, 0);
			zephir_check_call_status();
			while (1) {
				ZEPHIR_CALL_METHOD(&_3$$5, &keys, "valid", NULL, 0);
				zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(&key, &keys, "key", NULL, 0);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_1$$5, &keys, "current", NULL, 0);
				zephir_check_call_status();
					zephir_array_unset(&keys, &key, PH_SEPARATE);
					ZEPHIR_CALL_FUNCTION(NULL, "xcache_unset", &_6, 120, &key);
					zephir_check_call_status();
