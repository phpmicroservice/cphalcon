...
ZEPHIR_INIT_CLASS(Phalcon_Validation) {
	...
	zend_declare_property_null(phalcon_validation_ce, SL("_combinedFieldsValidators"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Validation, __construct) {
	...
	if (zephir_fast_count_int(&validators)) {
		ZEPHIR_INIT_VAR(&_0$$3);
		ZEPHIR_INIT_NVAR(&_0$$3);
		zephir_create_closure_ex(&_0$$3, NULL, phalcon_1__closure_ce, SL("__invoke"));
		ZEPHIR_CALL_FUNCTION(&_1$$3, "array_filter", NULL, 431, &validators, &_0$$3);
		...
		ZEPHIR_CALL_FUNCTION(&_3$$3, "array_filter", NULL, 431, &validators, &_2$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation, validate) {
	...
	zephir_is_iterable(&validators, 0, "phalcon/validation.zep", 161);
	if (Z_TYPE_P(&validators) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&validators), _1)
		{
			...
			zephir_array_fetch_long(&field, &scope, 0, PH_NOISY, "phalcon/validation.zep", 137);
			ZEPHIR_OBS_NVAR(&validator);
			zephir_array_fetch_long(&validator, &scope, 1, PH_NOISY, "phalcon/validation.zep", 138);
	...
	ZEPHIR_INIT_NVAR(&scope);
	zephir_is_iterable(&combinedFieldsValidators, 0, "phalcon/validation.zep", 193);
	if (Z_TYPE_P(&combinedFieldsValidators) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&combinedFieldsValidators), _12)
		{
			...
			ZEPHIR_OBS_NVAR(&field);
			zephir_array_fetch_long(&field, &scope, 0, PH_NOISY, "phalcon/validation.zep", 166);
			ZEPHIR_OBS_NVAR(&validator);
			zephir_array_fetch_long(&validator, &scope, 1, PH_NOISY, "phalcon/validation.zep", 167);
			if (Z_TYPE_P(&validator) != IS_OBJECT) {
...
PHP_METHOD(Phalcon_Validation, check) {
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *data = NULL, data_sub, *entity = NULL, entity_sub, __$null, _0, _1;
	zval *this_ptr = getThis();
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "validate", NULL, 0, data, entity);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation, add) {
	...
	if (Z_TYPE_P(field) == IS_ARRAY) {
		if (zephir_instance_of_ev(validator, phalcon_validation_combinedfieldsvalidator_ce)) {
			...
			zephir_is_iterable(field, 0, "phalcon/validation.zep", 232);
			if (Z_TYPE_P(field) == IS_ARRAY) {
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "Field must be passed as array of fields or string", "phalcon/validation.zep", 238);
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Validation, rules) {
	...
	zephir_is_iterable(&validators, 0, "phalcon/validation.zep", 264);
	if (Z_TYPE_P(&validators) == IS_ARRAY) {
		...
		while (1) {
			...
			zephir_check_call_status();
				if (zephir_instance_of_ev(&validator, phalcon_validation_validatorinterface_ce)) {
					ZEPHIR_CALL_METHOD(NULL, this_ptr, "add", &_2, 200, field, &validator);
					zephir_check_call_status();
				}
			ZEPHIR_CALL_METHOD(NULL, &validators, "next", NULL, 0);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation, setFilters) {
	...
	if (Z_TYPE_P(field) == IS_ARRAY) {
		zephir_is_iterable(field, 0, "phalcon/validation.zep", 281);
		if (Z_TYPE_P(field) == IS_ARRAY) {
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "Field must be passed as array of fields or string.", "phalcon/validation.zep", 286);
		return;
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Validation, setEntity) {
	...
	if (Z_TYPE_P(entity) != IS_OBJECT) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STRW(phalcon_validation_exception_ce, "Entity must be an object", "phalcon/validation.zep", 329);
		return;
	}
...
PHP_METHOD(Phalcon_Validation, setDefaultMessages) {
	...
	zephir_create_array(&defaultMessages, 25, 0);
	...
	ZEPHIR_INIT_VAR(&_0);
	zephir_fast_array_merge(&_0, &defaultMessages, &messages);
...
PHP_METHOD(Phalcon_Validation, getDefaultMessage) {
	...
	if (zephir_array_isset_fetch(&defaultMessage, &_0, &type, 1)) {
...
PHP_METHOD(Phalcon_Validation, appendMessage) {
	...
	ZEPHIR_CALL_METHOD(NULL, &messages, "appendmessage", NULL, 432, message);
	zephir_check_call_status();
	zephir_update_property_zval(this_ptr, SL("_messages"), &messages);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Validation, bind) {
	...
	zephir_fetch_params_without_memory_grow(2, 0, &entity, &data);



	if (Z_TYPE_P(entity) != IS_OBJECT) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STRW(phalcon_validation_exception_ce, "Entity must be an object", "phalcon/validation.zep", 467);
		return;
	}
	...
	if (_0) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STRW(phalcon_validation_exception_ce, "Data to validate must be an array or object", "phalcon/validation.zep", 471);
		return;
	}
	zephir_update_property_zval(this_ptr, SL("_entity"), entity);
	zephir_update_property_zval(this_ptr, SL("_data"), data);
...
PHP_METHOD(Phalcon_Validation, getValue) {
	...
	if (Z_TYPE_P(&entity) == IS_OBJECT) {
		...
		if (_0$$10) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "There is no data to validate", "phalcon/validation.zep", 516);
			return;
		}
		...
		if (Z_TYPE_P(&data) == IS_ARRAY) {
			if (zephir_array_isset(&data, &field)) {
				ZEPHIR_OBS_NVAR(&value);
				zephir_array_fetch(&value, &data, &field, PH_NOISY, "phalcon/validation.zep", 528);
			}
		} else {
	...
	if (zephir_array_isset_fetch(&fieldFilters, &filters, &field, 0)) {
		if (zephir_is_true(&fieldFilters)) {
			...
			if (Z_TYPE_P(&dependencyInjector) != IS_OBJECT) {
				...
				zephir_check_call_status();
				if (Z_TYPE_P(&dependencyInjector) != IS_OBJECT) {
					ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "A dependency injector is required to obtain the 'filter' service", "phalcon/validation.zep", 553);
					return;
				}
			}
			...
			if (Z_TYPE_P(&filterService) != IS_OBJECT) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_validation_exception_ce, "Returned 'filter' service is invalid", "phalcon/validation.zep", 559);
				return;
			}
			...
			if (Z_TYPE_P(&entity) == IS_OBJECT) {
				...
				if ((zephir_method_exists(&entity, &method)  == SUCCESS)) {
					...
					if ((zephir_method_exists_ex(&entity, ZEND_STRL("writeattribute")) == SUCCESS)) {
						ZEPHIR_CALL_METHOD(NULL, &entity, "writeattribute", NULL, 0, &field, &value);
						zephir_check_call_status();
					} else {
						ZEPHIR_CALL_FUNCTION(&_4$$28, "property_exists", NULL, 311, &entity, &field);
						zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation, preChecking) {
	...
	zval _13$$18;
	zend_bool _7$$12, _8$$12, _9$$15, _10$$15, _11$$9;
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zephir_fcall_cache_entry *_2 = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *field, field_sub, *validator, validator_sub, __$true, singleField, allowEmpty, emptyValue, value, result, *_0$$3, _1$$3, _3$$8, _4$$8, *_5$$11, _6$$11, raw$$9, _12$$18, _14$$18, _15$$18;
	zval *this_ptr = getThis();
	...
	if (Z_TYPE_P(field) == IS_ARRAY) {
		zephir_is_iterable(field, 0, "phalcon/validation.zep", 608);
		if (Z_TYPE_P(field) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(field), _0$$3)
			{
				ZEPHIR_INIT_NVAR(&singleField);
				ZVAL_COPY(&singleField, _0$$3);
				ZEPHIR_CALL_METHOD(&result, this_ptr, "prechecking", &_2, 433, &singleField, validator);
				zephir_check_call_status();
		...
		if (zephir_is_true(&allowEmpty)) {
			...
			if (Z_TYPE_P(&allowEmpty) == IS_ARRAY) {
				zephir_is_iterable(&allowEmpty, 0, "phalcon/validation.zep", 628);
				if (Z_TYPE_P(&allowEmpty) == IS_ARRAY) {
					ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&allowEmpty), _5$$11)
					{
						ZEPHIR_INIT_NVAR(&emptyValue);
						ZVAL_COPY(&emptyValue, _5$$11);
						_7$$12 = Z_TYPE_P(&emptyValue) == IS_OBJECT;
						if (_7$$12) {
							_7$$12 = Z_TYPE_P(&value) == IS_OBJECT;
						}
						_8$$12 = _7$$12;
						if (_8$$12) {
							_8$$12 = ZEPHIR_IS_EQUAL(&emptyValue, &value);
						}
					...
					while (1) {
						...
						zephir_check_call_status();
							_9$$15 = Z_TYPE_P(&emptyValue) == IS_OBJECT;
							if (_9$$15) {
								_9$$15 = Z_TYPE_P(&value) == IS_OBJECT;
							}
							_10$$15 = _9$$15;
							if (_10$$15) {
								_10$$15 = ZEPHIR_IS_EQUAL(&emptyValue, &value);
							}
							if (ZEPHIR_IS_IDENTICAL(&emptyValue, &value)) {
								RETURN_MM_BOOL(1);
							} else if (_10$$15) {
			...
			_11$$9 = Z_TYPE_P(&value) == IS_OBJECT;
			if (_11$$9) {
				_11$$9 = (zephir_method_exists_ex(&value, ZEND_STRL("__tostring")) == SUCCESS);
			}
			if (UNEXPECTED(_11$$9)) {
				ZEPHIR_CALL_METHOD(&_12$$18, &value, "__tostring", NULL, 0);
				zephir_check_call_status();
				ZEPHIR_INIT_VAR(&raw$$9);
				zephir_fast_strtolower(&raw$$9, &_12$$18);
				ZEPHIR_INIT_VAR(&_13$$18);
				zephir_create_array(&_13$$18, 3, 0);
				ZEPHIR_INIT_VAR(&_14$$18);
				ZVAL_STRING(&_14$$18, "null");
				zephir_array_fast_append(&_13$$18, &_14$$18);
				ZEPHIR_INIT_NVAR(&_14$$18);
				ZVAL_STRING(&_14$$18, "false");
				zephir_array_fast_append(&_13$$18, &_14$$18);
				ZEPHIR_INIT_NVAR(&_14$$18);
				ZVAL_STRING(&_14$$18, "");
				zephir_array_fast_append(&_13$$18, &_14$$18);
				ZEPHIR_CALL_FUNCTION(&_15$$18, "in_array", NULL, 362, &raw$$9, &_13$$18, &__$true);
				zephir_check_call_status();
