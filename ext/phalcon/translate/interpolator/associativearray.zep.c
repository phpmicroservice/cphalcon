...
ZEPHIR_INIT_CLASS(Phalcon_Translate_Interpolator_AssociativeArray) {
	...
	zend_class_implements(phalcon_translate_interpolator_associativearray_ce, 1, phalcon_translate_interpolatorinterface_ce);
	return SUCCESS;
	...
	if (_0) {
		...
		if (Z_TYPE_P(placeholders) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(placeholders), _3$$3, _4$$3, _1$$3)
			{
				...
				zephir_fast_str_replace(&_5$$4, &_6$$4, &value, &translation);
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					zephir_fast_str_replace(&_7$$5, &_8$$5, &value, &translation);
