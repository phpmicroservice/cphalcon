...
ZEPHIR_INIT_CLASS(Phalcon_Db_Adapter) {
	 ...
	 */
	zend_declare_property_null(phalcon_db_adapter_ce, SL("_eventsManager"), ZEND_ACC_PROTECTED);
	 ...
	 */
	zend_declare_property_bool(phalcon_db_adapter_ce, SL("_transactionsWithSavepoints"), 0, ZEND_ACC_PROTECTED);
	...
	zend_class_implements(phalcon_db_adapter_ce, 1, phalcon_events_eventsawareinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Phalcon_Db_Adapter, __construct) {
	...
	zval *descriptor_param = NULL, dialectClass, connectionId, _0, _1$$3, _2$$3, _3$$4, _4$$4;
	zval descriptor;
	zval *this_ptr = getThis();
	...
	zephir_update_static_property_ce(phalcon_db_adapter_ce, ZEND_STRL("_connectionConsecutive"), &_0);
	ZEPHIR_OBS_VAR(&dialectClass);
	if (!(zephir_array_isset_string_fetch(&dialectClass, &descriptor, SL("dialectClass"), 0))) {
		ZEPHIR_INIT_VAR(&_1$$3);
		zephir_read_property(&_2$$3, this_ptr, SL("_dialectType"), PH_NOISY_CC | PH_READONLY);
	...
	if (Z_TYPE_P(&dialectClass) == IS_STRING) {
		...
		if (zephir_has_constructor(&_3$$4)) {
			ZEPHIR_CALL_METHOD(NULL, &_3$$4, "__construct", NULL, 0);
			zephir_check_call_status();
		}
		zephir_update_property_zval(this_ptr, SL("_dialect"), &_3$$4);
	} else {
...
PHP_METHOD(Phalcon_Db_Adapter, fetchAll) {
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *sqlQuery_param = NULL, *fetchMode = NULL, fetchMode_sub, *bindParams = NULL, bindParams_sub, *bindTypes = NULL, bindTypes_sub, __$null, results, result;
	zval sqlQuery;
	zval *this_ptr = getThis();
...
PHP_METHOD(Phalcon_Db_Adapter, fetchColumn) {
	...
	if (_1) {
		_1 = zephir_array_isset_fetch(&columnValue, &row, column, 1);
	}
	if (_1) {
...
PHP_METHOD(Phalcon_Db_Adapter, insert) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval values;
	zval *table, table_sub, *values_param = NULL, *fields = NULL, fields_sub, *dataTypes = NULL, dataTypes_sub, __$null, placeholders, insertValues, bindDataTypes, bindType, position, value, escapedTable, joinedValues, escapedFields, field, insertSql, *_2, _3, _0$$3, _1$$3, _9$$8, _10$$9, _14$$16, _15$$17, *_16$$20, _17$$20, _20$$20, _18$$21, _19$$22;
	zval *this_ptr = getThis();
	...
	if (!(zephir_fast_count_int(&values))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$3, "phalcon/db/adapter.zep", 310);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	zephir_is_iterable(&values, 0, "phalcon/db/adapter.zep", 344);
	if (Z_TYPE_P(&values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&values), _4, _5, _2)
		{
			...
			if (_6$$4) {
				...
				zephir_array_append(&placeholders, &_7$$5, PH_SEPARATE, "phalcon/db/adapter.zep", 323);
			} else {
				...
				if (Z_TYPE_P(&value) == IS_NULL) {
					...
					zephir_array_append(&placeholders, &_9$$8, PH_SEPARATE, "phalcon/db/adapter.zep", 330);
				} else {
					ZEPHIR_INIT_NVAR(&_10$$9);
					ZVAL_STRING(&_10$$9, "?");
					zephir_array_append(&placeholders, &_10$$9, PH_SEPARATE, "phalcon/db/adapter.zep", 332);
					zephir_array_append(&insertValues, &value, PH_SEPARATE, "phalcon/db/adapter.zep", 333);
					if (Z_TYPE_P(dataTypes) == IS_ARRAY) {
						ZEPHIR_OBS_NVAR(&bindType);
						if (!(zephir_array_isset_fetch(&bindType, dataTypes, &position, 0))) {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$12) {
					...
					zephir_array_append(&placeholders, &_12$$13, PH_SEPARATE, "phalcon/db/adapter.zep", 323);
				} else {
					if (Z_TYPE_P(&value) == IS_OBJECT) {
					...
					if (Z_TYPE_P(&value) == IS_NULL) {
						...
						zephir_array_append(&placeholders, &_14$$16, PH_SEPARATE, "phalcon/db/adapter.zep", 330);
					} else {
						ZEPHIR_INIT_NVAR(&_15$$17);
						ZVAL_STRING(&_15$$17, "?");
						zephir_array_append(&placeholders, &_15$$17, PH_SEPARATE, "phalcon/db/adapter.zep", 332);
						zephir_array_append(&insertValues, &value, PH_SEPARATE, "phalcon/db/adapter.zep", 333);
						if (Z_TYPE_P(dataTypes) == IS_ARRAY) {
							ZEPHIR_OBS_NVAR(&bindType);
							if (!(zephir_array_isset_fetch(&bindType, dataTypes, &position, 0))) {
	...
	if (Z_TYPE_P(fields) == IS_ARRAY) {
		...
		zephir_is_iterable(fields, 0, "phalcon/db/adapter.zep", 356);
		if (Z_TYPE_P(fields) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(fields), _16$$20)
			{
				...
				zephir_check_call_status();
				zephir_array_append(&escapedFields, &_18$$21, PH_SEPARATE, "phalcon/db/adapter.zep", 353);
			} ZEND_HASH_FOREACH_END();
		} else {
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					zephir_check_call_status();
					zephir_array_append(&escapedFields, &_19$$22, PH_SEPARATE, "phalcon/db/adapter.zep", 353);
		...
		zephir_fast_join_str(&_20$$20, SL(", "), &escapedFields);
...
PHP_METHOD(Phalcon_Db_Adapter, insertAsDict) {
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *table, table_sub, *data, data_sub, *dataTypes = NULL, dataTypes_sub, __$null, values, fields, field, value, *_1, _2;
	zval *this_ptr = getThis();
	...
	zephir_is_iterable(data, 0, "phalcon/db/adapter.zep", 407);
...
PHP_METHOD(Phalcon_Db_Adapter, update) {
	...
	ZEPHIR_INIT_VAR(&bindDataTypes);
	array_init(&bindDataTypes);
	zephir_is_iterable(values, 0, "phalcon/db/adapter.zep", 494);
	if (Z_TYPE_P(values) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(values), _2, _3, _0)
		{
			...
			if (!(zephir_array_isset_fetch(&field, fields, &position, 0))) {
			...
			if (_5$$3) {
				...
				zephir_array_append(&placeholders, &_7$$5, PH_SEPARATE, "phalcon/db/adapter.zep", 473);
			} else {
				if (Z_TYPE_P(&value) == IS_OBJECT) {
					zephir_get_strval(&_8$$7, &value);
					ZEPHIR_CPY_WRT(&value, &_8$$7);
				...
				if (Z_TYPE_P(&value) == IS_NULL) {
					...
					zephir_array_append(&placeholders, &_9$$8, PH_SEPARATE, "phalcon/db/adapter.zep", 480);
				} else {
					...
					zephir_array_append(&placeholders, &_10$$9, PH_SEPARATE, "phalcon/db/adapter.zep", 489);
				}
			}
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$12) {
					...
					zephir_array_append(&placeholders, &_13$$14, PH_SEPARATE, "phalcon/db/adapter.zep", 473);
				} else {
					if (Z_TYPE_P(&value) == IS_OBJECT) {
					...
					if (Z_TYPE_P(&value) == IS_NULL) {
						...
						zephir_array_append(&placeholders, &_15$$17, PH_SEPARATE, "phalcon/db/adapter.zep", 480);
					} else {
						zephir_array_append(&updateValues, &value, PH_SEPARATE, "phalcon/db/adapter.zep", 482);
						if (Z_TYPE_P(dataTypes) == IS_ARRAY) {
							ZEPHIR_OBS_NVAR(&bindType);
						...
						zephir_array_append(&placeholders, &_16$$18, PH_SEPARATE, "phalcon/db/adapter.zep", 489);
					}
				}
			ZEPHIR_CALL_METHOD(NULL, values, "next", NULL, 0);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Adapter, updateAsDict) {
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *table, table_sub, *data, data_sub, *whereCondition = NULL, whereCondition_sub, *dataTypes = NULL, dataTypes_sub, __$null, values, fields, field, value, *_1, _2;
	zval *this_ptr = getThis();
	...
	zephir_is_iterable(data, 0, "phalcon/db/adapter.zep", 589);
	if (Z_TYPE_P(data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(data), _3, _4, _1)
		{
			ZEPHIR_INIT_NVAR(&field);
...
PHP_METHOD(Phalcon_Db_Adapter, escapeIdentifier) {
	...
	if (Z_TYPE_P(identifier) == IS_ARRAY) {
		...
		zephir_array_fetch_long(&_2$$3, identifier, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 651);
		ZEPHIR_CALL_METHOD(&_1$$3, &_0$$3, "escape", NULL, 0, &_2$$3);
		zephir_check_call_status();
		zephir_read_property(&_3$$3, this_ptr, SL("_dialect"), PH_NOISY_CC | PH_READONLY);
		zephir_array_fetch_long(&_5$$3, identifier, 1, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 651);
...
PHP_METHOD(Phalcon_Db_Adapter, tableExists) {
	...
	zephir_check_call_status();
	zephir_array_fetch_long(&_4, &_0, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 691);
...
PHP_METHOD(Phalcon_Db_Adapter, viewExists) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "fetchone", NULL, 0, &_2, &_3);
	zephir_check_call_status();
	zephir_array_fetch_long(&_4, &_0, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 705);
	RETURN_MM_BOOL(ZEPHIR_GT_LONG(&_4, 0));
...
PHP_METHOD(Phalcon_Db_Adapter, listTables) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&_0, 0, "phalcon/db/adapter.zep", 867);
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _4)
		{
			ZEPHIR_INIT_NVAR(&table);
			ZVAL_COPY(&table, _4);
			zephir_array_fetch_long(&_6$$3, &table, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 865);
			zephir_array_append(&allTables, &_6$$3, PH_SEPARATE, "phalcon/db/adapter.zep", 865);
		} ZEND_HASH_FOREACH_END();
	} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				zephir_array_fetch_long(&_7$$4, &table, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 865);
				zephir_array_append(&allTables, &_7$$4, PH_SEPARATE, "phalcon/db/adapter.zep", 865);
...
PHP_METHOD(Phalcon_Db_Adapter, listViews) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "fetchall", NULL, 0, &_2, &_3);
	zephir_check_call_status();
	zephir_is_iterable(&_0, 0, "phalcon/db/adapter.zep", 887);
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _4)
		{
			ZEPHIR_INIT_NVAR(&table);
			ZVAL_COPY(&table, _4);
			zephir_array_fetch_long(&_6$$3, &table, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 885);
			zephir_array_append(&allTables, &_6$$3, PH_SEPARATE, "phalcon/db/adapter.zep", 885);
		} ZEND_HASH_FOREACH_END();
	} else {
		ZEPHIR_CALL_METHOD(NULL, &_0, "rewind", NULL, 0);
		zephir_check_call_status();
		while (1) {
			...
			ZEPHIR_CALL_METHOD(&table, &_0, "current", NULL, 0);
			zephir_check_call_status();
				zephir_array_fetch_long(&_7$$4, &table, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 885);
				zephir_array_append(&allTables, &_7$$4, PH_SEPARATE, "phalcon/db/adapter.zep", 885);
			ZEPHIR_CALL_METHOD(NULL, &_0, "next", NULL, 0);
			zephir_check_call_status();
		}
	}
...
PHP_METHOD(Phalcon_Db_Adapter, describeIndexes) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "fetchall", NULL, 0, &_2, &_3);
	zephir_check_call_status();
	zephir_is_iterable(&_0, 0, "phalcon/db/adapter.zep", 921);
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _4)
		{
			ZEPHIR_INIT_NVAR(&index);
			ZVAL_COPY(&index, _4);
			zephir_array_fetch_long(&keyName, &index, 2, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 910);
			if (!(zephir_array_isset(&indexes, &keyName))) {
				ZEPHIR_INIT_NVAR(&columns);
				array_init(&columns);
			} else {
				ZEPHIR_OBS_NVAR(&columns);
				zephir_array_fetch(&columns, &indexes, &keyName, PH_NOISY, "phalcon/db/adapter.zep", 914);
			}
			zephir_array_fetch_long(&_6$$3, &index, 4, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 917);
			zephir_array_append(&columns, &_6$$3, PH_SEPARATE, "phalcon/db/adapter.zep", 917);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_array_fetch_long(&keyName, &index, 2, PH_NOISY, "phalcon/db/adapter.zep", 910);
				if (!(zephir_array_isset(&indexes, &keyName))) {
					ZEPHIR_INIT_NVAR(&columns);
					array_init(&columns);
				} else {
				...
				zephir_array_fetch_long(&_7$$6, &index, 4, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 917);
				zephir_array_append(&columns, &_7$$6, PH_SEPARATE, "phalcon/db/adapter.zep", 917);
				zephir_array_update_zval(&indexes, &keyName, &columns, PH_COPY | PH_SEPARATE);
			ZEPHIR_CALL_METHOD(NULL, &_0, "next", NULL, 0);
			zephir_check_call_status();
		}
	...
	zephir_is_iterable(&indexes, 0, "phalcon/db/adapter.zep", 930);
	if (Z_TYPE_P(&indexes) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&indexes), _10, _11, _8)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &_12$$9, "__construct", &_13, 19, &name, &indexColumns);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &_14$$10, "__construct", &_13, 19, &name, &indexColumns);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Adapter, describeReferences) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "fetchall", NULL, 0, &_2, &_3);
	zephir_check_call_status();
	zephir_is_iterable(&_0, 0, "phalcon/db/adapter.zep", 976);
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _4)
		{
			ZEPHIR_INIT_NVAR(&reference);
			ZVAL_COPY(&reference, _4);
			zephir_array_fetch_long(&constraintName, &reference, 2, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 952);
			if (!(zephir_array_isset(&references, &constraintName))) {
				ZEPHIR_OBS_NVAR(&referencedSchema);
				zephir_array_fetch_long(&referencedSchema, &reference, 3, PH_NOISY, "phalcon/db/adapter.zep", 954);
				ZEPHIR_OBS_NVAR(&referencedTable);
				zephir_array_fetch_long(&referencedTable, &reference, 4, PH_NOISY, "phalcon/db/adapter.zep", 955);
				ZEPHIR_INIT_NVAR(&columns);
				array_init(&columns);
				ZEPHIR_INIT_NVAR(&referencedColumns);
				array_init(&referencedColumns);
			} else {
				zephir_array_fetch(&_6$$5, &references, &constraintName, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 959);
				ZEPHIR_OBS_NVAR(&referencedSchema);
				zephir_array_fetch_string(&referencedSchema, &_6$$5, SL("referencedSchema"), PH_NOISY, "phalcon/db/adapter.zep", 959);
				zephir_array_fetch(&_7$$5, &references, &constraintName, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 960);
				ZEPHIR_OBS_NVAR(&referencedTable);
				zephir_array_fetch_string(&referencedTable, &_7$$5, SL("referencedTable"), PH_NOISY, "phalcon/db/adapter.zep", 960);
				zephir_array_fetch(&_8$$5, &references, &constraintName, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 961);
				ZEPHIR_OBS_NVAR(&columns);
				zephir_array_fetch_string(&columns, &_8$$5, SL("columns"), PH_NOISY, "phalcon/db/adapter.zep", 961);
				zephir_array_fetch(&_9$$5, &references, &constraintName, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 962);
				ZEPHIR_OBS_NVAR(&referencedColumns);
				zephir_array_fetch_string(&referencedColumns, &_9$$5, SL("referencedColumns"), PH_NOISY, "phalcon/db/adapter.zep", 962);
			}
			zephir_array_fetch_long(&_10$$3, &reference, 1, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 965);
			zephir_array_append(&columns, &_10$$3, PH_SEPARATE, "phalcon/db/adapter.zep", 965);
			zephir_array_fetch_long(&_11$$3, &reference, 5, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 966);
			zephir_array_append(&referencedColumns, &_11$$3, PH_SEPARATE, "phalcon/db/adapter.zep", 966);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_array_fetch_long(&constraintName, &reference, 2, PH_NOISY, "phalcon/db/adapter.zep", 952);
				if (!(zephir_array_isset(&references, &constraintName))) {
					ZEPHIR_OBS_NVAR(&referencedSchema);
					zephir_array_fetch_long(&referencedSchema, &reference, 3, PH_NOISY, "phalcon/db/adapter.zep", 954);
					ZEPHIR_OBS_NVAR(&referencedTable);
					...
					zephir_array_fetch(&_13$$8, &references, &constraintName, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 959);
					ZEPHIR_OBS_NVAR(&referencedSchema);
					zephir_array_fetch_string(&referencedSchema, &_13$$8, SL("referencedSchema"), PH_NOISY, "phalcon/db/adapter.zep", 959);
					zephir_array_fetch(&_14$$8, &references, &constraintName, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 960);
					ZEPHIR_OBS_NVAR(&referencedTable);
					zephir_array_fetch_string(&referencedTable, &_14$$8, SL("referencedTable"), PH_NOISY, "phalcon/db/adapter.zep", 960);
					zephir_array_fetch(&_15$$8, &references, &constraintName, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 961);
					ZEPHIR_OBS_NVAR(&columns);
					zephir_array_fetch_string(&columns, &_15$$8, SL("columns"), PH_NOISY, "phalcon/db/adapter.zep", 961);
					zephir_array_fetch(&_16$$8, &references, &constraintName, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 962);
					ZEPHIR_OBS_NVAR(&referencedColumns);
					zephir_array_fetch_string(&referencedColumns, &_16$$8, SL("referencedColumns"), PH_NOISY, "phalcon/db/adapter.zep", 962);
				}
				zephir_array_fetch_long(&_17$$6, &reference, 1, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 965);
				zephir_array_append(&columns, &_17$$6, PH_SEPARATE, "phalcon/db/adapter.zep", 965);
				zephir_array_fetch_long(&_18$$6, &reference, 5, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 966);
				zephir_array_append(&referencedColumns, &_18$$6, PH_SEPARATE, "phalcon/db/adapter.zep", 966);
	...
	zephir_is_iterable(&references, 0, "phalcon/db/adapter.zep", 986);
	if (Z_TYPE_P(&references) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&references), _22, _23, _20)
		{
			...
			zephir_array_fetch_string(&_26$$9, &arrayReference, SL("referencedSchema"), PH_NOISY, "phalcon/db/adapter.zep", 979);
			zephir_array_update_string(&_25$$9, SL("referencedSchema"), &_26$$9, PH_COPY | PH_SEPARATE);
			ZEPHIR_OBS_NVAR(&_26$$9);
			zephir_array_fetch_string(&_26$$9, &arrayReference, SL("referencedTable"), PH_NOISY, "phalcon/db/adapter.zep", 980);
			zephir_array_update_string(&_25$$9, SL("referencedTable"), &_26$$9, PH_COPY | PH_SEPARATE);
			ZEPHIR_OBS_NVAR(&_26$$9);
			zephir_array_fetch_string(&_26$$9, &arrayReference, SL("columns"), PH_NOISY, "phalcon/db/adapter.zep", 981);
			zephir_array_update_string(&_25$$9, SL("columns"), &_26$$9, PH_COPY | PH_SEPARATE);
			ZEPHIR_OBS_NVAR(&_26$$9);
			zephir_array_fetch_string(&_26$$9, &arrayReference, SL("referencedColumns"), PH_NOISY, "phalcon/db/adapter.zep", 983);
			zephir_array_update_string(&_25$$9, SL("referencedColumns"), &_26$$9, PH_COPY | PH_SEPARATE);
			ZEPHIR_CALL_METHOD(NULL, &_24$$9, "__construct", &_27, 20, &name, &_25$$9);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				zephir_array_fetch_string(&_30$$10, &arrayReference, SL("referencedSchema"), PH_NOISY, "phalcon/db/adapter.zep", 979);
				zephir_array_update_string(&_29$$10, SL("referencedSchema"), &_30$$10, PH_COPY | PH_SEPARATE);
				ZEPHIR_OBS_NVAR(&_30$$10);
				zephir_array_fetch_string(&_30$$10, &arrayReference, SL("referencedTable"), PH_NOISY, "phalcon/db/adapter.zep", 980);
				zephir_array_update_string(&_29$$10, SL("referencedTable"), &_30$$10, PH_COPY | PH_SEPARATE);
				ZEPHIR_OBS_NVAR(&_30$$10);
				zephir_array_fetch_string(&_30$$10, &arrayReference, SL("columns"), PH_NOISY, "phalcon/db/adapter.zep", 981);
				zephir_array_update_string(&_29$$10, SL("columns"), &_30$$10, PH_COPY | PH_SEPARATE);
				ZEPHIR_OBS_NVAR(&_30$$10);
				zephir_array_fetch_string(&_30$$10, &arrayReference, SL("referencedColumns"), PH_NOISY, "phalcon/db/adapter.zep", 983);
				zephir_array_update_string(&_29$$10, SL("referencedColumns"), &_30$$10, PH_COPY | PH_SEPARATE);
				ZEPHIR_CALL_METHOD(NULL, &_28$$10, "__construct", &_27, 20, &name, &_29$$10);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Db_Adapter, tableOptions) {
	...
	if (zephir_is_true(&sql)) {
		...
		zephir_check_call_status();
		zephir_array_fetch_long(&_3$$3, &_1$$3, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter.zep", 1004);
		RETURN_CTOR(&_3$$3);
	}
	array_init(return_value);
	RETURN_MM();
