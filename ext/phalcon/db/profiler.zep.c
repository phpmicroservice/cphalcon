...
PHP_METHOD(Phalcon_Db_Profiler, startProfile) {
	...
	zval *sqlStatement, sqlStatement_sub, *sqlVariables = NULL, sqlVariables_sub, *sqlBindTypes = NULL, sqlBindTypes_sub, __$true, __$null, activeProfile, _0;
	...
	if (Z_TYPE_P(sqlVariables) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(NULL, &activeProfile, "setsqlvariables", NULL, 157, sqlVariables);
		zephir_check_call_status();
	}
