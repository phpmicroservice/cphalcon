...
ZEPHIR_INIT_CLASS(Phalcon_Db_Result_Pdo) {
	 ...
	 * Active fetch mode
	 */
	zend_declare_property_long(phalcon_db_result_pdo_ce, SL("_fetchMode"), 5, ZEND_ACC_PROTECTED);
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&rowCount)) {
		...
		if (ZEPHIR_IS_FALSE_IDENTICAL(&rowCount)) {
			...
			if (!(zephir_start_with_str(&sqlStatement, SL("SELECT COUNT(*) ")))) {
				...
				zephir_preg_match(&_2$$6, &_3$$6, &sqlStatement, &matches, 0, 0 , 0 );
				if (zephir_is_true(&_2$$6)) {
					zephir_array_fetch_long(&_4$$7, &matches, 1, PH_NOISY | PH_READONLY, "phalcon/db/result/pdo.zep", 238);
					...
					zephir_array_fetch_string(&rowCount, &row, SL("numrows"), PH_NOISY, "phalcon/db/result/pdo.zep", 244);
				}
			} else {
