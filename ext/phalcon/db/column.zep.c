...
ZEPHIR_INIT_CLASS(Phalcon_Db_Column) {
	 ...
	 * @var string
	 */
	zend_declare_property_null(phalcon_db_column_ce, SL("_name"), ZEND_ACC_PROTECTED);
	 ...
	 * @var int
	 */
	zend_declare_property_long(phalcon_db_column_ce, SL("_size"), 0, ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Db_Column, __set_state) {
	...
	if (zephir_array_isset_string_fetch(&scale, &data, SL("_scale"), 1)) {
		zephir_array_fetch_string(&_1$$12, &definition, SL("type"), PH_NOISY | PH_READONLY, "phalcon/db/column.zep", 523);
		do {
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 154, &columnName, &definition);
