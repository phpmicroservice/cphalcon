...
PHP_METHOD(Phalcon_Forms_Element, __construct) {
	...
	zval *name_param = NULL, *attributes = NULL, attributes_sub, __$null, _0, _1;
	zval name;
...
PHP_METHOD(Phalcon_Forms_Element, addValidators) {
	...
	if (merge) {
		...
		if (Z_TYPE_P(&currentValidators) == IS_ARRAY) {
			...
			zephir_fast_array_merge(&mergedValidators, &currentValidators, &validators);
		}
	} else {
...
PHP_METHOD(Phalcon_Forms_Element, prepareAttributes) {
	...
	if (Z_TYPE_P(&defaultAttributes) == IS_ARRAY) {
		...
		zephir_fast_array_merge(&mergedAttributes, &defaultAttributes, &widgetAttributes);
	} else {
...
PHP_METHOD(Phalcon_Forms_Element, label) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *attributes = NULL, attributes_sub, __$null, internalAttributes, label, name, code, _2, _0$$6, _4$$7, _5$$8;
	zval *this_ptr = getThis();
	...
	if (_3) {
		...
		zephir_concat_self(&code, &_4$$7);
	} else {
		ZEPHIR_INIT_VAR(&_5$$8);
		ZEPHIR_CONCAT_SVS(&_5$$8, ">", &name, "</label>");
		zephir_concat_self(&code, &_5$$8);
	}
	RETURN_CCTOR(&code);
...
PHP_METHOD(Phalcon_Forms_Element, getValue) {
	...
	if (Z_TYPE_P(&form) == IS_OBJECT) {
		...
		if (_0$$3) {
			ZEPHIR_CALL_CE_STATIC(&_1$$3, phalcon_tag_ce, "hasvalue", &_2, 0, &name);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Forms_Element, clear) {
	...
	ZVAL_NULL(&_2);
	ZEPHIR_CALL_CE_STATIC(NULL, phalcon_tag_ce, "setdefault", &_0, 0, &_1, &_2);
	zephir_check_call_status();
	RETURN_THIS();
