...
PHP_METHOD(Phalcon_Events_Manager, attach) {
	...
	if (!(zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0))) {
		...
		if (zephir_is_true(&_1$$4)) {
			...
			ZVAL_LONG(&_2$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "setextractflags", NULL, 189, &_2$$5);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "insert", NULL, 190, handler, &_3$$7);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Events_Manager, detach) {
	...
	if (zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0)) {
		if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
			...
			ZVAL_LONG(&_1$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "setextractflags", NULL, 189, &_1$$5);
			zephir_check_call_status();
			ZVAL_LONG(&_1$$5, 3);
			...
			while (1) {
				...
				zephir_check_call_status();
				zephir_array_fetch_string(&_5$$6, &data, SL("data"), PH_NOISY | PH_READONLY, "phalcon/events/manager.zep", 116);
				if (!ZEPHIR_IS_IDENTICAL(&_5$$6, handler)) {
					zephir_array_fetch_string(&_6$$7, &data, SL("data"), PH_NOISY | PH_READONLY, "phalcon/events/manager.zep", 117);
					zephir_array_fetch_string(&_7$$7, &data, SL("priority"), PH_NOISY | PH_READONLY, "phalcon/events/manager.zep", 117);
					ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "insert", &_8, 190, &_6$$7, &_7$$7);
					zephir_check_call_status();
				}
			}
			zephir_update_property_array(this_ptr, SL("_events"), &eventType, &newPriorityQueue);
		} else {
			ZEPHIR_CALL_FUNCTION(&key, "array_search", NULL, 191, handler, &priorityQueue, &__$true);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, fireQueue) {
	...
	if (Z_TYPE_P(queue) != IS_ARRAY) {
		if (Z_TYPE_P(queue) == IS_OBJECT) {
			if (!(zephir_instance_of_ev(queue, spl_ce_SplPriorityQueue))) {
				ZEPHIR_INIT_VAR(&_0$$5);
				object_init_ex(&_0$$5, phalcon_events_exception_ce);
				ZEPHIR_INIT_VAR(&_1$$5);
				zephir_get_class(&_1$$5, queue, 0);
				ZEPHIR_INIT_VAR(&_2$$5);
				ZVAL_STRING(&_2$$5, "Unexpected value type: expected object of type SplPriorityQueue, %s given");
				ZEPHIR_CALL_FUNCTION(&_3$$5, "sprintf", NULL, 151, &_2$$5, &_1$$5);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(NULL, &_0$$5, "__construct", NULL, 4, &_3$$5);
				zephir_check_call_status();
				zephir_throw_exception_debug(&_0$$5, "phalcon/events/manager.zep", 210);
				ZEPHIR_MM_RESTORE();
				return;
			}
		} else {
	...
	if (Z_TYPE_P(queue) == IS_OBJECT) {
		...
		zephir_is_iterable(queue, 0, "phalcon/events/manager.zep", 358);
		if (Z_TYPE_P(queue) == IS_ARRAY) {
				...
				if (Z_TYPE_P(&handler) == IS_OBJECT) {
					if (zephir_instance_of_ev(&handler, zend_ce_closure)) {
						if (Z_TYPE_P(&arguments) == IS_NULL) {
							...
							zephir_create_array(&_14$$25, 3, 0);
					...
					if (Z_TYPE_P(&handler) == IS_OBJECT) {
						if (zephir_instance_of_ev(&handler, zend_ce_closure)) {
							if (Z_TYPE_P(&arguments) == IS_NULL) {
								...
								zephir_create_array(&_17$$37, 3, 0);
...
PHP_METHOD(Phalcon_Events_Manager, fire) {
	...
	if (!(zephir_memnstr_str(&eventType, SL(":"), "phalcon/events/manager.zep", 386))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$4, "phalcon/events/manager.zep", 387);
		ZEPHIR_MM_RESTORE();
		return;
	}
	...
	ZEPHIR_OBS_VAR(&type);
	zephir_array_fetch_long(&type, &eventParts, 0, PH_NOISY, "phalcon/events/manager.zep", 391);
	ZEPHIR_OBS_VAR(&eventName);
	zephir_array_fetch_long(&eventName, &eventParts, 1, PH_NOISY, "phalcon/events/manager.zep", 392);
	ZEPHIR_INIT_VAR(&status);
	ZVAL_NULL(&status);
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &type, 0)) {
		...
		if (_3$$6) {
			...
			ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 192, &eventName, source, data, &_4$$7);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 193, &fireEvents, &event);
			zephir_check_call_status();
		}
	}
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &eventType, 0)) {
		...
		if (_5$$8) {
			if (Z_TYPE_P(&event) == IS_NULL) {
				...
				ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 192, &eventName, source, data, &_6$$10);
				zephir_check_call_status();
			}
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 193, &fireEvents, &event);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, getListeners) {
	...
	if (Z_TYPE_P(&events) == IS_ARRAY) {
		if (zephir_array_isset_fetch(&fireEvents, &events, &type, 1)) {
