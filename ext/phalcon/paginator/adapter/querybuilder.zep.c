...
ZEPHIR_INIT_CLASS(Phalcon_Paginator_Adapter_QueryBuilder) {
	 ...
	 */
	zend_declare_property_null(phalcon_paginator_adapter_querybuilder_ce, SL("_config"), ZEND_ACC_PROTECTED);
	 ...
	 */
	zend_declare_property_null(phalcon_paginator_adapter_querybuilder_ce, SL("_columns"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Paginator_Adapter_QueryBuilder, paginate) {
	...
	if (hasGroup) {
		if (Z_TYPE_P(&groups) == IS_ARRAY) {
			...
			zephir_fast_join_str(&groupColumn$$11, SL(", "), &groups);
		} else {
	...
	if (hasHaving) {
		...
		if (Z_TYPE_P(&modelClass) == IS_ARRAY) {
			...
			zephir_check_call_status();
			zephir_array_fetch_long(&_12$$17, &_11$$17, 0, PH_NOISY | PH_READONLY, "phalcon/paginator/adapter/querybuilder.zep", 241);
		...
		zephir_check_call_status();
		zephir_array_fetch_string(&_16$$16, &sql, SL("sql"), PH_NOISY | PH_READONLY, "phalcon/paginator/adapter/querybuilder.zep", 247);
		ZEPHIR_INIT_VAR(&_17$$16);
		ZEPHIR_CONCAT_SVS(&_17$$16, "SELECT COUNT(*) as \"rowcount\" FROM (", &_16$$16, ") as T1");
		zephir_array_fetch_string(&_18$$16, &sql, SL("bind"), PH_NOISY | PH_READONLY, "phalcon/paginator/adapter/querybuilder.zep", 247);
		ZVAL_LONG(&_19$$16, 2);
		ZEPHIR_CALL_METHOD(&row, &db, "fetchone", NULL, 0, &_17$$16, &_19$$16, &_18$$16);
		zephir_check_call_status();
		if (zephir_is_true(&row)) {
			zephir_array_fetch_string(&_20$$16, &row, SL("rowcount"), PH_NOISY | PH_READONLY, "phalcon/paginator/adapter/querybuilder.zep", 248);
			ZVAL_LONG(&rowcount, zephir_get_intval(&_20$$16));
		} else {
			ZVAL_LONG(&rowcount, 0);
		}
		ZEPHIR_INIT_VAR(&_21$$16);
		div_function(&_21$$16, &rowcount, &limit);
		ZVAL_DOUBLE(&_19$$16, zephir_ceil(&_21$$16));
		...
		div_function(&_23$$18, &rowcount, &limit);
		ZVAL_DOUBLE(&_24$$18, zephir_ceil(&_23$$18));
	...
	ZEPHIR_INIT_ZVAL_NREF(_10);
	ZVAL_LONG(&_10, previous);
	zephir_update_property_zval(&page, SL("previous"), &_10);
	ZEPHIR_INIT_ZVAL_NREF(_10);
