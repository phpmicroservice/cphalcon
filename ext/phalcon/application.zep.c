...
ZEPHIR_INIT_CLASS(Phalcon_Application) {
	...
	zend_declare_property_null(phalcon_application_ce, SL("_eventsManager"), ZEND_ACC_PROTECTED);

	zend_declare_property_null(phalcon_application_ce, SL("_dependencyInjector"), ZEND_ACC_PROTECTED);
	...
	zend_class_implements(phalcon_application_ce, 1, phalcon_events_eventsawareinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Phalcon_Application, __construct) {
	...
	zephir_fetch_params_without_memory_grow(0, 1, &dependencyInjector);
...
PHP_METHOD(Phalcon_Application, getModule) {
	...
	if (!(zephir_array_isset_fetch(&module, &_0, &name, 0))) {
		...
		zephir_throw_exception_debug(&_1$$3, "phalcon/application.zep", 123);
		ZEPHIR_MM_RESTORE();
		return;
	}
	RETURN_CCTOR(&module);
